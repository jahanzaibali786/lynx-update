...
class CashReciptVoucherController extends Controller
{
    ...
    public function index(Request $request)
    {
        if(\Auth::user()->can('manage journal entry'))
        {
            ...
            if (\Auth::user()->type == 'company') {
                ...
                $query = JournalEntry::where('created_by', '=', \Auth::user()->creatorId())->where('voucher_type','CRV');
            }else{
                $branches = User::where('id', '=', \Auth::user()->ownedId())->get()->pluck('name', 'id');
                $branches->prepend('Select Branch', '');
                $query = JournalEntry::where('owned_by', '=', \Auth::user()->ownedId())->where('voucher_type','CRV');
            }
            ...
            return view('cash-recipt-voucher.index', compact('journalEntries','branches', 'startDate', 'endDate', 'voucherSeriesFilter'));
        }
        else
        {
    ...
    public function create()
    {
        if(\Auth::user()->can('create journal entry'))
        {
            ...
            $journalId = $this->CRVNumber();

            return view('cash-recipt-voucher.create', compact('chartAccounts', 'subAccounts', 'journalId'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create invoice'))
        {
            ...
            $journal->journal_id   = $this->CRVNumber();
            ...
            \DB::commit();
                return response()->json([
                    'status' => 'success',
                    'message' => __('Cash Recipt Voucher successfully created.'),
                    'redirect' => route('cash-recipt-voucher.show', $journal->id)
                ]);
                // return redirect()->route('cash-recipt-voucher.index')->with('success', __('Cash Recipt Voucher successfully created.'));
            } catch (\Exception $e) {
                \DB::rollback();
                return response()->json([
                    'status' => 'error',
    ...
    public function show($journalEntry)
    {
        ...
        {
            $journalEntry = JournalEntry::with('accounts.receiptheads.challan.student','accounts.receiptheads.challan.enrollstudent')->where('id', $journalEntry)->first();
            if($journalEntry->created_by == \Auth::user()->creatorId())
            {
                $accounts = $journalEntry->accounts;
                $settings = Utility::settings();

                return view('cash-recipt-voucher.view', compact('journalEntry', 'accounts', 'settings'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function edit($journalEntry)
    {
        if(\Auth::user()->can('edit journal entry'))
        {
           ...
           $chartAccounts = ChartOfAccount::select(\DB::raw('CONCAT(chart_of_accounts.code, " - ", chart_of_accounts.name) AS code_name,chart_of_accounts.id, chart_of_accounts.code,  chart_of_accounts.parent'))
            ...
            $subAccounts->where('chart_of_accounts.created_by', \Auth::user()->creatorId());
            $subAccounts = $subAccounts->get()->toArray();
            return view('cash-recipt-voucher.edit', compact('chartAccounts', 'subAccounts', 'journalEntry'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
    ...
    public function update(Request $request, $journalEntry)
    {
        if(\Auth::user()->can('edit journal entry'))
        {
            ...
            if($journalEntry->created_by == \Auth::user()->creatorId())
            {
                ...
                return redirect()->route('cash-recipt-voucher.index')->with('success', __('Cash Recipt Voucher successfully updated.'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function destroy($journalEntry)
    {
        ...
        if(\Auth::user()->can('delete journal entry'))
        {
            ...
            if($journalEntry->created_by == \Auth::user()->creatorId())
            {
                ...
                return redirect()->route('cash-recipt-voucher.index')->with('success', __('Cash Recipt Voucher successfully deleted.'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
    ...
    function CRVNumber()
    {
        $latest = JournalEntry::where('owned_by', '=', \Auth::user()->ownedId())->where('voucher_type','CRV')->latest()->first();
        if(!$latest)
        {
            return 1;
        }
    ...
    public function accountDestroy(Request $request)
    {
        ...
        if(\Auth::user()->can('delete journal entry'))
        {
            ...
            return redirect()->back()->with('success', __('Cash Recipt Voucher account successfully deleted.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
