...
class WarehouseTransferController extends Controller
{
    ...
    {
        $warehouse_transfers = WarehouseTransfer::where('created_by', '=', \Auth::user()->creatorId())->paginate(12);
    ...
    public function getquantity(Request $request)
    {
        ...
        {
            $pro_qty = WarehouseProduct::where('created_by', '=', \Auth::user()->creatorId())->where('warehouse_id',$request->warehouse_id)
                ->get()->pluck('quantity', 'product_id')->toArray();
        }
        else
        {
            $pro_qty = WarehouseProduct::where('created_by', '=', \Auth::user()->creatorId())
                        ->where('warehouse_id',$request->warehouse_id)->where('product_id', $request->product_id)
                        ->get()->pluck('quantity');
