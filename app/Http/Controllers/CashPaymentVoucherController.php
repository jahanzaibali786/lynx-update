...
class CashPaymentVoucherController extends Controller
{
    ...
    public function index(Request $request)
    {
        if(\Auth::user()->can('manage journal entry'))
        {
            ...
            return view('cash-payment-voucher.index', compact('journalEntries','branches', 'startDate', 'endDate', 'voucherSeriesFilter'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create invoice'))
        {
            ...
            \DB::commit();
                return response()->json([
                    'status' => 'success',
                    'message' => __('Cash Payment Voucher successfully created.'),
                    'redirect' => route('cash-payment-voucher.show', $journal->id)
                ]);
    ...
    public function edit($journalEntry)
    {
        if(\Auth::user()->can('edit journal entry'))
        {
            ...
            $subAccounts->where('chart_of_accounts.created_by', \Auth::user()->creatorId());
            $subAccounts = $subAccounts->get()->toArray();
            return view('cash-payment-voucher.edit', compact('chartAccounts', 'subAccounts', 'journalEntry'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
