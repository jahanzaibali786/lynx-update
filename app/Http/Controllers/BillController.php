            ...
            try {
                ...
                for($i = 0; $i < count($products); $i++)
                {
                    ...
                    if(!empty($products[$i]['chart_account_id'])){
                        ...
                        $billAccount->price             = @$products[$i]['amount'] == null ? 0 : @$products[$i]['amount'];
                        $billAccount->description       = $products[$i]['description'];
                        $billAccount->type              = 'Bill';
                        $billAccount->bill_product_id   = $products[$i]['item'];
                ...
                $data['reference'] = $bill->id;
                $data['description'] = $bill->bill_id;
                $data['category'] = 'Bill';
                ...
                try {
                    ...
                    for ($i = 0; $i < count($products); $i++)
                    {
                        ...
                        if ($billProduct == null)
                        {
                            ...
                            $voucher = JournalEntry::where('category', 'Bill')->where('description', "Bill id : {$bill->id}")->first();
                            $bill_pod_price = $products[$i]['price'] - $billProduct->price;
                            $tax_diff = $products[$i]['itemTaxPrice'] - (($billProduct->price * $billProduct->quantity) - $billProduct->discount) * ($products[$i]['itemTaxRate'] /100);
                            $product_ite = ProductService::where('id',$billProduct->product_id)->first();
                            $product_ite_new = ProductService::where('id',$products[$i]['items'])->first();
                            $item1 = JournalItem::where('journal',$voucher->id)->where('account',$product_ite->sale_chartaccount_id)->update(['credit' => $products[$i]['price'],'account' => $product_ite_new->sale_chartaccount_id]);
                            $bil_act = BillAccount::where('bill_product_id',$billProduct->product_id)->where('ref_id',$bill->id)->first();
                            $item2 = JournalItem::where('journal',$voucher->id)->where('account',$bil_act->chart_account_id)->update(['credit' => $products[$i]['amount'],'account' => $products[$i]['chart_account_id']]);
                            $bill_acc_price = $bil_act->price-$products[$i]['amount'];
                            $types = ChartOfAccountType::where('owned_by', '=', $bill->owned_by)->where('name','Liabilities')->first();
                            if($types){
                            ...
                            $item_last->debit = $item_last->debit + ((float)$bill_pod_price+ (float)$bill_acc_price+(float)$tax_diff);
                            $item_last->save();
                        ...
                        if (isset($products[$i]['items'])) {
                            ...
                            if(empty($products[$i]['id'])){
                                $voucher = JournalEntry::where('category', 'Bill')->where('description', "Bill id : {$bill->id}")->first();
                                $product_ad = ProductService::where('id',$products[$i]['items'])->first();
                                $journalItem              = new JournalItem();
                                $journalItem->journal     = $voucher->id;
                                $journalItem->account     = $product_ad->sale_chartaccount_id;
                                $journalItem->description = $product_ad->description;
                                $journalItem->debit       =  0;
                                $journalItem->credit      = (($products[$i]['quantity'] * $products[$i]['price']) - $products[$i]['discount']);
                                $journalItem->save();
                                $payable += (($products[$i]['quantity'] * $products[$i]['price']) - $products[$i]['discount']) + $products[$i]['itemTaxPrice'];
                                ...
                                if(!empty($products[$i]['chart_account_id'])){
                                        ...
                                        $journalItem->debit       =  0;
                                        $journalItem->credit      = @$products[$i]['amount'] == null ? 0 : @$products[$i]['amount'];
                                        $journalItem->save();
                                        $payable += $products[$i]['amount'];
                                }
                                $types = ChartOfAccountType::where('owned_by', '=', $bill->owned_by)->where('name','Liabilities')->first();
                                if($types){
                                    $sub_type = ChartOfAccountSubType::where('type', $types->id)->where('name','Current Liabilities')->first();
                                ...
                                $item_last = JournalItem::where('journal',$voucher->id)->where('account',$account->id)->first();
                                $item_last->debit = $item_last->debit + $payable;
                                $item_last->save();
                            }
                        }
                        ...
                        if(!empty($products[$i]['chart_account_id'])){
                            ...
                            if ($billAccount == null) {
                                $billAccount                    = new BillAccount();
                                $billAccount->chart_account_id = $products[$i]['chart_account_id'];
                                $billAccount->bill_product_id   = $products[$i]['items'];
                            }
                            else{
                                $billAccount->chart_account_id = $products[$i]['chart_account_id']; 
                                $billAccount->bill_product_id   = $products[$i]['items'];
    ...
    public function productDestroy(Request $request)
    {
        ...
        if(\Auth::user()->can('delete bill product'))
        {
            ...
            try {
                ...
                $product = ProductService::where('id',$billProduct->product_id)->first();
                $bil_act = BillAccount::where('bill_product_id',$billProduct->product_id)->where('ref_id',$bill->id)->first();
                $new_amount = $request->amount + $bil_act->price;
                $item = JournalItem::where('journal',$voucher->id)->where('account',$product->sale_chartaccount_id)->delete();
                $bil_act_del = BillAccount::where('bill_product_id',$billProduct->product_id)->where('ref_id',$bill->id)->delete();
                $item_bil_act = JournalItem::where('journal',$voucher->id)->where('account',$bil_act->chart_account_id)->delete();
                // dd($billProduct,$bill,$voucher,$bil_act,$billProduct->product_id,$bill->bill_id);
                $types = ChartOfAccountType::where('owned_by', '=', $bill->owned_by)->where('name','Liabilities')->first();
                if($types){
                    $sub_type = ChartOfAccountSubType::where('type', $types->id)->where('name','Current Liabilities')->first();
                    $account = ChartOfAccount::where('type', $types->id)->where('sub_type', $sub_type->id)->where('name','Account Payable')->first();
                ...
                $item_last = JournalItem::where('journal',$voucher->id)->where('account',$account->id)->first();
                $item_last->debit = $item_last->debit - $new_amount;
                $item_last->save();
