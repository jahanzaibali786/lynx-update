...
class AppraisalController extends Controller
{
    ...
    public function index(Request $request)
    {
        if(\Auth::user()->can('manage appraisal'))
        {
            ...
            if($user->type == 'Employee')
            {
                ...
                $query = Appraisal::with('branchuser')->where('owned_by', '=', \Auth::user()->creatorId())->where('branch', $employee->branch_id)->where('employee', $employee->id);
            }
            elseif($user->type == 'company')
            {
                ...
                $query = Appraisal::with('branchuser')->where('created_by', '=', \Auth::user()->creatorId());
            }else{
                $branches = User::where('id', '=', \Auth::user()->ownedId())->get()->pluck('name', 'id');
                $branches->prepend('Select Branch', '');
                $competencyCount = Competencies::where('owned_by', '=', $user->ownedId())->count();

                $query = Appraisal::with('branchuser')->where('owned_by', '=', $user->ownedId());
            }
    ...
    public function create()
    {
        if(\Auth::user()->can('create appraisal'))
        {
            if(\Auth::user()->type == 'company'){
                ...
                $brances = User::where('type', '=', 'branch')->get()->pluck('name', 'id');
                $brances->prepend(\Auth::user()->name, key: \Auth::user()->id);
                $brances->prepend('Select Branch', '');
            }else{
                $performance     = PerformanceType::where('owned_by', '=', \Auth::user()->ownedId())->get();
                $brances = User::where('id', '=', \Auth::user()->ownedId())->get()->pluck('name', 'id');
                $brances->prepend('Select Branch', '');
            }
    ...
    public function edit(Appraisal $appraisal)
    {
        if(\Auth::user()->can('edit appraisal'))
        {
            if(\Auth::user()->type == 'company'){
                $performance_types     = PerformanceType::where('created_by', '=', \Auth::user()->creatorId())->get();
                $brances = User::where('type', '=', 'branch')->where('created_by', '=', \Auth::user()->creatorId())->get();
                $brances->prepend(\Auth::user()->name, \Auth::user()->id);
                $ratings = json_decode($appraisal->rating,true);
            }else{
                $performance_types     = PerformanceType::where('owned_by', '=', \Auth::user()->ownedId())->get();
                $brances = User::where('type', '=', 'branch')->where('owned_by', '=', \Auth::user()->ownedId())->get();
                $ratings = json_decode($appraisal->rating,true);
            }
