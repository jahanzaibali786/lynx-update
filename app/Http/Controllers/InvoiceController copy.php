    ...
    public function create($customerId)
    {
        if (\Auth::user()->can('create invoice')) {
            ...
            if (\Auth::user()->type == 'company') {
                ...
                $branches->prepend(\Auth::user()->name, \Auth::user()->id);
                $branches->prepend('Select Branch', '');
                $stores = warehouse::where('created_by', '=', \Auth::user()->creatorId())->get()->pluck('name', 'id');
                ...
                ->where('warehouse_products.warehouse_id', 2)
                ...
                $branches = User::where('id', '=', \Auth::user()->ownedId())->get()->pluck('name', 'id');
                $branches->prepend('Select Branch', '');
                $stores = warehouse::where('owned_by', \Auth::user()->ownedId())->get()->pluck('name', 'id');
                $invoice_number = \Auth::user()->invoiceNumberFormat($this->invoiceNumber());
            ...
            $students = StudentRegistration::get()->pluck('stdname', 'id');

            return view('invoice.create', compact('customers', 'company','students', 'invoice_number', 'product_services', 'category', 'branches','stores', 'customFields', 'customerId'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create invoice')) {
            ...
            try {
                ...
                $invoice->customer_id    = $request->student_id;
                $invoice->status         = 0;
                ...
                $invoice->owned_by     = $request->branches;
                ...
                for ($i = 0; $i < count($products); $i++) {
                    ...
                    Utility::warehouse_quantity('minus',$products[$i]['quantity'],$products[$i]['item'],$request->store);
