...
class SiblingStudentExport implements FromView, WithEvents
{
    protected $branches;
    protected $groupedStudents;

    public function __construct($branches, $groupedStudents)
    {
     ...
     public function view(): View
    {
        $is_signature = false;
        $is_period = false;
        // $report_name = __('Student Sibling Report');
        $report_name = __('STUDENT SIBLING REPORT');
        $branch = 'All Branches';
        // Pass
        //  only the table-related data to the export view
        return view('student.exports.sibling_students', [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle("D8:J{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
                $sheet->getStyle("K8:M{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                $sheet->getStyle("K8:M{$lastDataRow}")->getNumberFormat()->setFormatCode('#,##0');
