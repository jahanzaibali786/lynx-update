...
class ProfessionWiseListingExport implements FromView, WithEvents
{
    ...
    public function __construct($groupedStudents, $branchName, $branches,$branchId)
    {
    ...
    public function view(): View
    {
        $is_signature = false;
        $is_period = false;
        $report_name = __('Profession Wise Listing Report');
        // Pass
        //  only the table-related data to the export view
        return view('student.exports.profession_wise_listing', [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle("N8:N{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle("P8:P{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle("D8:G{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
                $sheet->getStyle("I8:M{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
                $sheet->getStyle("O8:O{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
