...
public function registerEvents(): array
{
    return [
        AfterSheet::class => function (AfterSheet $event) {
            ...
            // ===== FIND AND STYLE BRANCH NAME ROWS & TOTAL ROWS =====
            for ($row = 10; $row <= $lastDataRow; $row++) {
                $cellValue = $sheet->getCell('A' . $row)->getValue();
                $cellB = $sheet->getCell('B' . $row)->getValue();
                
                // Detect branch name rows: A has value, B is empty (because of colspan in HTML)
                // Branch rows have data only in column A and rest are empty
                if (!empty($cellValue) && empty($cellB) && stripos($cellValue, 'Total') === false) {
                    // This is likely a branch name row
                    // Merge cells A to P (all 16 columns)
                    $sheet->mergeCells("A{$row}:{$highestColumnLetter}{$row}");
                    
                    // Apply styling to branch name row
                    $sheet->getStyle("A{$row}:{$highestColumnLetter}{$row}")->applyFromArray([
                        ...
                        'fill' => [
                            ...
                            'startColor' => [
                                'argb' => 'FFF0F0F0', // Light gray background
                            ],
                        ],
                    ]);
                }
                
                // Check if this row contains "Total" (Branch Total rows)
                if (stripos($cellValue, 'Total') !== false) {
                    // Merge cells A to N (columns 1-14) for colspan effect
            ...
            // $sheet->getStyle("A10:P{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
