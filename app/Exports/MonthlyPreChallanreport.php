    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // ── Cast values: col G → Excel date serial, col H+ → float ─
                // Pre-convert dateInput once outside the loop (not per-row)
                $excelDateSerial = ExcelDate::PHPToExcel(strtotime(\Carbon\Carbon::parse($this->dateInput)->format('Y-m-d')));

                for ($col = $numericStartCol; $col <= $highestColumnIndex; $col++) {

                    // ── Column G: write pre-computed date serial to all rows at once
                    if ($col === $dateColIndex) {
                        for ($row = 9; $row <= $lastDataRow; $row++) {
                            $rawValue = $sheet->getCellByColumnAndRow($col, $row)->getValue();
                            // Only process actual student data rows (skip branch/total rows which may be empty or text)
                ...
                // ── Last column override: text, left-aligned ─────────────
