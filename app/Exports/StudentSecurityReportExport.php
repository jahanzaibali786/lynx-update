...
class StudentSecurityReportExport implements FromView, WithEvents
{
    protected $records;
    ...
    public function __construct($branches, $records, $selectedBranchId, $heads, $journals = null, $request = null, $report_name, $params = null)
    {
        // Set memory and timeout first
        ini_set('memory_limit', '512M'); // Start with 512MB, adjust as needed
    ...
    public function view(): View
    {
        ...
        $branch = '';
        if (!empty($this->branches) && $this->selectedBranchId && isset($this->branches[$this->selectedBranchId])) {
            $branch = $this->branches[$this->selectedBranchId];
        }
        // Pass only the table-related data to the export view
        return view('studentReports.exports.studentsecurityreportexport', [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle("D10:F{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
                $sheet->getStyle("H10:H{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle("J10:K{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle("N10:N{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                $sheet->getStyle("P10:P{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
                ...
                $sheet->getStyle("I10:J{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                $sheet->getStyle("I10:J{$lastDataRow}")->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle("L10:M{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                $sheet->getStyle("L10:M{$lastDataRow}")->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle("O10:O{$lastDataRow}")->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                $sheet->getStyle("O10:O{$lastDataRow}")->getNumberFormat()->setFormatCode('#,##0');
