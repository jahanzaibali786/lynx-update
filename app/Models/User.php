...

{
    ...
    public function ownedBy()
    {
        ...
        {
            return User::find($this->owned_by);
        }
    }

    // public function ownedByName()
    // {
    //     $owner = $this->ownedBy();
...
//        return (($settings['site_currency_symbol_position'] == "pre") ? $settings['site_currency_symbol'] : '') . number_format($price, $settings['decimal_number']) . (($settings['site_currency_symbol_position'] == "post") ? $settings['site_currency_symbol'] : '');
    ...
    {
        return User::where('type', '=', 'client')->where('created_by', '=', $this->creatorId())->count();
    }
    ...
    public function countStudents()
    {
        ...
        {
            return StudentRegistration::where('created_by', '=', $this->creatorId())->where('student_status','Enrolled')->count();
        }
        else
        {
            return StudentRegistration::where('owned_by', '=', $this->ownedId())->where('student_status','Enrolled')->count();
        }
    }
    ...
    public function countCustomers()
    {
        ...
        {
            return Customer::where('owned_by', '=', $this->ownedId())->count();
        }
    }
    ...
    public function countVenders()
    {
        ...
        {
            return Vender::where('owned_by', '=', $this->ownedId())->count();
        }
    }
    public function countEmployee()
    {
        ...
        {
            return Employee::where('owned_by', '=', $this->ownedId())->count();
        }
    }
    ...
    public function countInvoices()
    {
        ...
        {
            return Invoice::where('owned_by', '=', $this->ownedId())->count();
        }
    }

    public function countChallans()
    {
        if($this->type == 'company' )
        {
            return Challans::where('created_by', '=', $this->creatorId())->count();
        }
        else
        {
            return Challans::where('owned_by', '=', $this->ownedId())->count();
        }
    }
    ...
    public function countVouchers()
    {
        if($this->type == 'company' )
        {
            return JournalEntry::where('created_by', '=', $this->creatorId())->count();
        }
        else
        {
            return JournalEntry::where('owned_by', '=', $this->ownedId())->count();
        }
    }
    ...
    public function countBills()
    {
        ...
        {
            return Bill::where('owned_by', '=', $this->ownedId())->count();
        }
    }
    ...
    {
        return User::where('id', $branch_id)->first();
    }
    ...
    public function getQualification($employeeId)
    {
        // Assuming 1-to-1 (latest qualification), or pick the logic you want
        $education = EmpEducation::where('emp_id', $employeeId)->orderByDesc('pass_date')->first();
        // dd($employeeId,$education);
        return $education ? $education->degree : null;
    }
     ...
     public static function employeeIdFormat($number)
    {
        ...
        return $settings["employee_prefix"] . sprintf("%04d", $number);
    }
    ...
    public static function newpriceFormat($price)
    {
        ...
        if ($settings === null) {
            $settings = Utility::settings(); // Load only once
        }

        $decimal = Utility::getValByName('decimal_number'); // Ideally cache this too

        $symbol = $settings['site_currency_symbol'] ?? '';
        $position = $settings['site_currency_symbol_position'] ?? 'pre';

        $formatted = number_format($price, $decimal);
