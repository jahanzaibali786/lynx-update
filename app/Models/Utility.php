...
class Utility extends Model
{
    ...
    public static function addNewData()
    {
        ...
        $companyNewPermission = [
            ...
            'approve stock transfer order',
            'reject stock transfer order',
            'convert stock transfer order to invoice',
            'manage grn',
            'create grn',
            'show grn',
            'edit grn',
    ...
    {
        // dd($from_warehouse, $to_warehouse, $product_id, $quantity, $delete);
    ...
    public static function addProductStock($product_id, $quantity, $type, $description, $type_id, $extra = [])
    {
        ...
        if (isset($extra['warehouse_id'])) {
            $stocks->warehouse_id = $extra['warehouse_id'];
        }
        if (isset($extra['unit_price'])) {
            $stocks->unit_price = $extra['unit_price'];
        }
        if (isset($extra['sale_price'])) {
            $stocks->sale_price = $extra['sale_price'];
        }
        if (isset($extra['remaining_qty'])) {
            $stocks->remaining_qty = $extra['remaining_qty'];
        }
        if (isset($extra['condition'])) {
            $stocks->condition = $extra['condition'];
        }
    ...
    public static function getAccountBalance($account_id, $start_date = null, $end_date = null, $branch = null)
    {
        ...
        $journalCredit = JournalItem::select('journal_entries.journal_id', 'journal_entries.voucher_type', 'journal_entries.date as transaction_date', 'journal_items.*')
        ...
        $journalDebit = JournalItem::select('journal_entries.journal_id', 'journal_entries.voucher_type', 'journal_entries.date as transaction_date', 'journal_items.*')
            ->leftjoin('journal_entries', 'journal_entries.id', 'journal_items.journal')->where('journal_entries.created_by', '=', \Auth::user()->creatorId())->where('account', $account_id);
    ...
    public static function old_trialBalance($account_id, $start)
    {
        // dd($account_id, $start);
    ...
    public static function oldtrialBalance($account_id, $start, $branch = null)
    {
        $journalItem = JournalItem::select('chart_of_accounts.id', 'chart_of_accounts.code', 'chart_of_accounts.name', \DB::raw('sum(debit) as totalDebit'), \DB::raw('sum(credit) as totalCredit'));
        $journalItem->leftjoin('journal_entries', 'journal_entries.id', 'journal_items.journal');
        $journalItem->leftjoin('chart_of_accounts', 'journal_items.account', 'chart_of_accounts.id');
        ...
        $total = array_merge($journalItem);

        return $total;

    }

    public static function trialBalance($account_id, $start, $end, $branch = null)
    {
        $journalItem = JournalItem::select('chart_of_accounts.id', 'chart_of_accounts.code', 'chart_of_accounts.name', \DB::raw('sum(debit) as totalDebit'), \DB::raw('sum(credit) as totalCredit'));
        $journalItem->leftjoin('journal_entries', 'journal_entries.id', 'journal_items.journal');
        $journalItem->leftjoin('chart_of_accounts', 'journal_items.account', 'chart_of_accounts.id');
    ...
    public static function virtual_office_branch($user, $created_by)
    {
        ...
        $types = ChartOfAccountType::where('created_by', '=', $created_by)->where('name', 'Income')->first();
        if ($types) {
            $sub_type = ChartOfAccountSubType::where('type', $types->id)->where('name', 'Sales Revenue')->first();
        }
    ...
    public static function security_services_branch($user, $created_by)
    {
        ...
        $types = ChartOfAccountType::where('created_by', '=', $created_by)->where('name', 'Liabilities')->first();
        if ($types) {
            $sub_type = ChartOfAccountSubType::where('type', $types->id)->where('name', 'Current Liabilities')->first();
        }
    ...
    public static function jrentry($data)
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($data['items']); $i++) {
                $head_id = FeeHead::where('id', $data['items'][$i]['head'])->first();
                ...
                if ($data['items'][$i]['concession'] > 0) {
                    // dd($data['items'][$i]['concession']);
                    $account_name = ChartOfAccount::where('id', $head_id->discount_account_id)->first();
                    // dd($account_name,$head_id,$data['items'][$i]['concession']);
                    $journalItem = new JournalItem;
                    $journalItem->journal = $journal->id;
                    $journalItem->account = @$account_name->id;
                ...
                //     $account = ChartOfAccount::where('type', $types->id)->where('sub_type', $sub_type->id)->where('name',$account_name->name)->first();
                // }
                // if($account){
                //     $journalItem              = new JournalItem();
                //     $journalItem->journal     = $journal->id;
                //     $journalItem->account     = @$account->id;
                //     $journalItem->description = 'Reciveable of Challan id : '.$data['id'];
                //     $journalItem->credit       =  0;
                //     $journalItem->debit      = ($data['items'][$i]['quantity'] * $data['items'][$i]['price']) - $data['items'][$i]['concession'];
                //     $journalItem->save();
                // }else{
                //     $reciveable += ($data['items'][$i]['quantity'] * $data['items'][$i]['price']) - $data['items'][$i]['concession'];
                // }
    ...
    public static function bankTransferJvEntry($data)
    {
        ...
        try {
            ...
            // dd($data['date']);
            $createdatJ = ! empty($data['date'])
                ? Carbon::parse($data['date'])->format('Y-m-d H:i:s')
                : null;

            $updatedatj = ! empty($data['date'])
                ? Carbon::parse($data['date'])->format('Y-m-d H:i:s')
                : null;
            $journal = new JournalEntry;
            $journal->journal_id = $latest;
    ...
    // Salary jr voucher
    public static function Salaryjrentry($data)
    {
        DB::beginTransaction();
        try {
            ...
            foreach ($data['accounts'] as $acc) {
                $type = ChartOfAccountType::firstOrCreate(
                    ['name' => $acc['type'], 'created_by' => $data['created_by']],
                    ['description' => $acc['type']]
                );
                $sub_type = ChartOfAccountSubType::firstOrCreate(
                    ['type' => $type->id, 'name' => $acc['sub_type']],
                    ['description' => $acc['sub_type']]
                );
    ...
    public static function Salaryjrentryvoucher($data)
    {
        DB::beginTransaction();
        try {
            $latest = JournalEntry::where('owned_by', '=', $data['owned_by'])->where('voucher_type', 'JV')->orderBy('id', 'Desc')->first();
    ...
    public static function Salarycrvvoucher($data)
    {
        DB::beginTransaction();
        try {
    ...
    public static function Salarybrvvoucher($data)
    {
        DB::beginTransaction();
        try {
    ...
    // adj voucher entry
    public static function adj_voucher($data)
    {
        DB::beginTransaction();
        try {
            $latest = JournalEntry::where('owned_by', '=', $data['owned_by'])->where('voucher_type', 'JV')->latest()->first();
            ...
            for ($i = 0; $i < count($data['items']); $i++) {
                $head_id = FeeHead::where('id', $data['items'][$i]['head'])->first();
                $account_name = ChartOfAccount::where('id', $head_id->receivable_account_id)->first();
                $journalItem = new JournalItem;
                $journalItem->journal = $journal->id;
                $journalItem->account = @$account_name->id;
                $journalItem->head = $data['items'][$i]['head'];
            ...
            $account_sec = ChartOfAccount::where('id', $head->receivable_account_id)->first();
            $journalItem = new JournalItem;
            $journalItem->journal = $journal->id;
    ...
    // {
    //     $latest = JournalEntry::where('owned_by', '=', $data['owned_by'])->where('voucher_type', 'BRV')->orderBy('id', 'desc')->first();
    //     if (! $latest) {
    //         $latest = 1;
    //     } else {
    //         $latest = $latest->journal_id + 1;
    ...
    //     $journal->date = $data['date'];
    //     $journal->reference = $data['reference'];
    //     $journal->description = 'Challan id : '.$data['id'];
    //     $journal->reference_id = $data['id'];
    ...
    //     StudentReceipt::where('id', $data['recipt'])->update([
    ...
    //         $journalItem->description = 'Receive of Challan no : '.$data['no'].' - '.@$data['std_name'].' - '.@$data['fee_month'].' - '.@$data['branch_name'].' - '.@$data['bank_name'];
    //         $journalItem->user_id = $data['user_id'];
    //         $journalItem->user_type = 'Student';
    //         $journalItem->entry_id = @$data['prod_id'];
    //         $journalItem->types = 'Challan Payment';
    //         $journalItem->branch_id = @$data['branch_id'];
    //         $quantity = $data['items'][$i]['quantity'];
    //         $price = str_replace(',', '', $data['items'][$i]['price']);
    //         $concession = str_replace(',', '', $data['items'][$i]['concession']);
    //         if (!is_numeric($quantity) || !is_numeric($price) || !is_numeric($concession)) {
    //             dd('Non-numeric value encountered in quantity, price, or concession.', $data);
    //         }
    //         $journalItem->credit = ((float) $quantity * (float) $price) - (float) $concession;
    //         $journalItem->debit = 0;
    //         $journalItem->save();
    ...
    // {
    //     // dd($data);
    //     $latest = JournalEntry::where('owned_by', '=', $data['owned_by'])->where('voucher_type', 'CRV')->orderBy('id', 'desc')->first();
    //     if (! $latest) {
    //         $latest = 1;
    //     } else {
    //         $latest = $latest->journal_id + 1;
    ...
    //     $journal->reference = $data['reference'];
    //     $journal->bank_id = $data['bank_id'];
    //     $journal->description = 'Challan id : '.@$data['no'];
    //     $journal->reference_id = $data['id'];
    //     $journal->user_id = $data['user_id'];
    //     $journal->user_type = 'Student';
    //     $journal->category = $data['category'];
    ...
    //         $journalItem->types = 'Challan Payment';
    //         $journalItem->branch_id = @$data['branch_id'];
    //         $journalItem->description = 'Recive of Challan no : '.@$data['no'].' - '.@$data['std_name'].' - '.@$data['fee_month'].' - '.@$data['branch_name'].' - '.@$data['bank_name'];
    //         $quantity = $data['items'][$i]['quantity'];
    //         $price = str_replace(',', '', $data['items'][$i]['price']);
    //         $concession = str_replace(',', '', $data['items'][$i]['concession']);
    //         if (! is_numeric($quantity) || ! is_numeric($price) || ! is_numeric($concession)) {
    ...
    public static function bpv_entry($data)
    {
        ...
        if (@$data['category'] == 'Purchase') {
            ...
            for ($i = 0; $i < count($data['items']); $i++) {
                ...
                $journalItem->credit = 0;
                $journalItem->debit = $data['items'][$i]['amount'];
                $journalItem->save();
            }
            ...
            $types = ChartOfAccountType::where('owned_by', '=', $data['owned_by'])->where('name', 'Liabilities')->first();
            if ($types) {
                $sub_type = ChartOfAccountSubType::where('type', $types->id)->where('name', 'Current Liabilities')->first();
    ...
    public static function jr_exp_entry($data)
    {
        ...
        for ($i = 0; $i < count($data['items']); $i++) {
            if (! empty($data['items'][$i]['item'])) {
                ...
                $journalItem->description = $product->description;
                $journalItem->debit = 0;
                $journalItem->credit = $data['items'][$i]['quantity'] * $data['items'][$i]['price'] - $data['items'][$i]['discount'];
                $journalItem->save();
                $payable += ($data['items'][$i]['quantity'] * $data['items'][$i]['price'] - $data['items'][$i]['discount']) + $data['items'][$i]['itemTaxPrice'];
            ...
            if (! empty($data['items'][$i]['chart_account_id'])) {
                ...
                $journalItem->description = $data['items'][$i]['description'];
                $journalItem->debit = 0;
                $journalItem->credit = @$data['items'][$i]['amount'] == null ? 0 : @$data['items'][$i]['amount'];
                $journalItem->save();
                $payable += $data['items'][$i]['amount'];
            }
        ...
        $journalItem->journal = $journal->id;
        $journalItem->account = $account->id;
        $journalItem->description = 'Account Payable';
        $journalItem->debit = $payable;
        $journalItem->credit = 0;
        $journalItem->save();
    ...
    {
        $inv = StudyPackChallans::where('id', $data['id'])->first();
        // dd($inv);
        $bankto = BankAccount::find($data['account_id']);
        if (strtolower($bankto->bank_name) == 'cash' || strtolower($bankto->holder_name) == 'cash') {
            $vocher_type = 'CRV';
        } else {
            $vocher_type = 'BRV';
        }
        ...
        if ($types) {
            $sub_type = ChartOfAccountSubType::where('type', $types->id)->where('name', 'Receivables')->first();
            $account = ChartOfAccount::where('type', $types->id)->where('sub_type', $sub_type->id)->where('name', 'Studypack Receivables')->first();
        }
    ...
    // {
    //     // Get the latest journal entry ID
    //     $latest = JournalEntry::where('owned_by', $data['owned_by'])
    //         ->where('voucher_type', 'JV')
    //         ->latest()
    //         ->first();
    ...
    //     $journal->date = $data['date'];
    //     $journal->reference = $data['reference'];
    //     $journal->description = 'Salary Id : ' . @$data['id'];
    //     $journal->reference_id = $data['id'];
    //     $journal->category = $data['category'];
    //     $journal->voucher_type = 'JV';
    //     $journal->owned_by = $data['owned_by'];
    ...
    //     foreach ($data['items'] as $item) {
    //         $product = EmployeeMonthlySalaryHeads::where('id', $item['prod_id'])->first();
    //         $emp_scale= EmployeePayscaleDetail::where('id',$product->scale_id)->first();
    //         dd($emp_scale,$product);
    //         if (!$product)
    //             continue;
    ...
    //     }
    //     $payables = [
    //         'Loan' => 'Loan Payable Account',
    //         'Security' => 'Security Payable Account',
    //         'Eboi' => 'Eboi Payable Account',
    //         'Pessi' => 'Pessi Payable Account',
    //     ];

    //     foreach ($payables as $payableType => $accountName) {
    //         $payableAccount = ChartOfAccount::where('created_by', $data['created_by'])
    //             ->where('name', $accountName)
    //             ->first();

    //         if (!$payableAccount) {
    //             $payableAccount = new ChartOfAccount();
    //             $payableAccount->name = $accountName;
    //             $payableAccount->code = '0';
    //             $payableAccount->type = 'Payable Type';
    //             $payableAccount->sub_type = 'Payable Sub Type';
    //             $payableAccount->description = $accountName;
    //             $payableAccount->is_enabled = 1;
    //             $payableAccount->created_by = \Auth::user()->creatorId();
    //             $payableAccount->save();
    //         }

    //         $payableAmount = EmployeeScale::where('employee_id', $data['id'])
    //             ->where('payable_type', $payableType)
    //             ->sum('amount');

    //         $journalItem = new JournalItem();
    //         $journalItem->journal = $journal->id;
    //         $journalItem->account = $payableAccount->id;
    //         $journalItem->description = $payableType . ' Payable';
    //         $journalItem->debit = $payableAmount;
    //         $journalItem->credit = 0;
    //         $journalItem->branch_id = $data['owned_by'];
    //         $journalItem->save();
    //     }
    ...
    //         $account = ChartOfAccount::where('type', $types->id)
    //             ->where('sub_type', $sub_type->id)
    //             ->where('name', 'Salary Receivables')
    //             ->first();
    //         if (!$account) {
    //             $account = new ChartOfAccount();
    //             $account->name = 'Salary Receivables';
    //             $account->code = '0';
    //             $account->type = $types->id;
    //             $account->sub_type = $sub_type->id;
    //             $account->description = 'Salary Receivables';
    //             $account->is_enabled = 1;
    //             $account->created_by = \Auth::user()->creatorId();
    //             $account->save();
    //         }
    ...
    //         $journalItem->journal = $journal->id;
    //         $journalItem->account = @$account->id;
    //         $journalItem->description = 'Salary Receivables';
    //         $journalItem->debit = $receivable + $totalTax;
    //         $journalItem->branch_id = $data['owned_by'];
    //         $journalItem->save();
    //     }
    ...
    // Invoice JV voucher working
    public static function invoicejv($data)
    {
        // dd($data);
        ...
        $journal->category = $data['category'];
        $journal->voucher_type = 'JV';
        $journal->owned_by = $from_store->owned_by;
        $journal->created_by = $data['created_by'];
        $journal->save();
        ...
        for ($i = 0; $i < count($data['items']); $i++) {
            ...
            $journalItem->debit = 0;
            $journalItem->save();
            $reciveable += ((floatval($data['items'][$i]['quantity']) * floatval($data['items'][$i]['price'])) - $data['items'][$i]['discount']);
        ...
        //         $journalItem->description = 'Tax on Purchase id : '.$data['no'];
        //         $journalItem->credit       =  0;
        ...
        if ($reciveable != 0) {
            ...
            if ($types) {
                $sub_type = ChartOfAccountSubType::where('type', $types->id)->where('name', 'Receivables')->first();
                $account = ChartOfAccount::where('type', $types->id)->where('sub_type', $sub_type->id)->where('name', 'Receivable study pack')->first();
            }

            if ($account) {
            } else {
                $account = new ChartOfAccount;
                $account->name = 'Receivable study pack';
                $account->code = '0';
                $account->type = $types->id;
                $account->sub_type = $sub_type->id;
                $account->description = 'Receivable study pack';
                $account->is_enabled = 1;
                $account->created_by = \Auth::user()->creatorId();
                $account->save();
            }
        ...
        $journal2->owned_by = $to_store->owned_by;
        $journal2->created_by = $data['created_by'];
        $journal2->save();
        ...
        for ($i = 0; $i < count($data['items']); $i++) {
            ...
            $journalItem->debit = ($data['items'][$i]['quantity'] * $data['items'][$i]['price']) - $data['items'][$i]['discount'];
            $journalItem->save();
            $reciveable2 += ((floatval($data['items'][$i]['quantity']) * floatval($data['items'][$i]['price'])) - $data['items'][$i]['discount']);
            // $tax += $data['items'][$i]['itemTaxPrice'];
        }
        // For Tax
        // if($tax != 0){
    ...
    // Invoice Pay voucher working
    public static function invoicepay($data)
    {
        // dd($data);
        $inv = Invoice::where('id', $data['id'])->first();
        $from_store = warehouse::where('id', $inv->from_store)->first();
        $bankto = BankAccount::find($data['to_bank']);
        if (strtolower($bankto->bank_name) == 'cash' || strtolower($bankto->holder_name) == 'cash') {
            $vocher_type = 'CRV';
        } else {
            $vocher_type = 'BRV';
        ...
        $journalItem->save();
        // to Store
        $to_store = warehouse::where('id', $inv->from_store)->first();
        $bankfrom = BankAccount::find($data['to_bank']);
        if (strtolower($bankfrom->bank_name) == 'cash' || strtolower($bankfrom->holder_name) == 'cash') {
            $vocher_type = 'CPV';
        } else {
            $vocher_type = 'BPV';
        }
