                    ...
                    r ? dragula(n, {
                        ...
                        if(stage_name == 'Achieved'){
                            $('#statuscontract'+id).empty().append(` <div class="badge-xs badge bg-primary p-0 rounded float-end mr-1"><a href="#" data-size="lg" data-url="{{ route('contract.create') }}" data-ajax-popup="true"
                                                 data-bs-title="{{ __('Create New Contract') }}" class="btn btn-sm btn-primary rounded">
                                                make contract
                                            </a></div>`);
                        }else{
...
@section('action-btn')
    <div class="float-end">
        ...
        <a href="{{ route('deals.list') }}" data-size="lg"  data-bs-title="{{__('List View')}}" class="btn btn-sm btn-primary">
            <i class="ti ti-list"></i>
        </a>
        <a href="#" data-size="lg" data-url="{{ route('deals.create') }}" data-ajax-popup="true"  data-bs-title="{{__('Create New Deal')}}" data-bs-toggle="{{__('Create Deal')}}" class="btn btn-sm btn-primary">
            Create
        </a>
    </div>
@endsection
                                            ...
                                            @if(Auth::user()->type != 'client')
                                                <div class="btn-group card-option">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        @can('edit deal')
                                                            <a href="#!" data-size="md" data-url="{{ URL::to('deals/'.$deal->id.'/labels') }}" data-ajax-popup="true" class="dropdown-item" data-bs-title="{{__('Labels')}}">
                                                                <i class="ti ti-bookmark"></i>
                                                                <span>{{__('Labels')}}</span>
                                                            </a>
                                                            <a href="#!" data-size="lg" data-url="{{ URL::to('deals/'.$deal->id.'/edit') }}" data-ajax-popup="true" class="dropdown-item" data-bs-title="{{__('Edit Deal')}}">
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex align-items-center justify-content-between mb-2">
                                            <ul class="list-inline mb-0">
                                                <li class="list-inline-item d-inline-flex align-items-center"  data-bs-title="{{__('Tasks')}}">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between">
                                            <ul class="list-inline mb-0">
                                                <li class="list-inline-item d-inline-flex align-items-center"  data-bs-title="{{__('Product')}}">
                                                    <i class="f-16 text-primary ti ti-shopping-cart"></i> {{count($products)}}
                                                </li>
                                                <li class="list-inline-item d-inline-flex align-items-center"  data-bs-title="{{__('Source')}}">
                                                    <i class="f-16 text-primary ti ti-social"></i>{{count($sources)}}
                                                </li>
                                            </ul>
                                            <div class="user-group">
                                                @foreach($deal->users as $user)
                                                    <img src="@if($user->avatar) {{asset('/storage/uploads/avatar/'.$user->avatar)}} @else {{asset('storage/uploads/avatar/avatar.png')}} @endif"   data-bs-title="{{$user->name}}">
                                                @endforeach
                                            </div>
                                        </div>
                                    </div>
