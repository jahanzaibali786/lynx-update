...
@section('action-btn')
    <div class="float-end">
        <a href="{{ route('deals.index') }}"  data-bs-title="{{__('Kanban View')}}" class="btn btn-sm btn-primary">
            <i class="ti ti-layout-grid"></i>
        </a>
        <a href="#" data-size="lg" data-url="{{ route('deals.create') }}" data-ajax-popup="true"  data-bs-title="{{__('Create New Deal')}}" class="btn btn-sm btn-primary">
            Create
        </a>
    </div>
@endsection
                                            ...
                                            <td>
                                                @foreach($deal->users as $user)
                                                    <a href="#" class="btn btn-sm mr-1 p-0 rounded-circle">
                                                        <img alt="image"  data-bs-title="{{$user->name}}" @if($user->avatar) src="{{asset('/storage/uploads/avatar/'.$user->avatar)}}" @else src="{{asset('/storage/uploads/avatar/avatar.png')}}" @endif class="rounded-circle " width="25" height="25">
                                                    </a>
                                                @endforeach
                                            </td>
                                                    ...
                                                    <span>
                                                        @can('view deal')
                                                            @if($deal->is_active)
                                                                ...
                                                                <a href="{{route('deals.show',$deal->id)}}" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-size="xl"  data-bs-title="{{__('View')}}" data-bs-toggle="{{__('Lead Detail')}}">
                                                        ...
                                                        @can('edit deal')
                                                            <div class="action-btn bg-info ms-2">
                                                                <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-url="{{ URL::to('deals/'.$deal->id.'/edit') }}" data-ajax-popup="true" data-size="lg"  data-bs-title="{{__('Edit')}}" data-bs-toggle="{{__('Lead Edit')}}">
