...
@section('action-btn')
    ...
    @can('create project task stage')
            <a href="#" data-url="{{ route('project-task-stages.create') }}"   data-bs-title="{{__('Create')}}" class="btn btn-sm btn-primary" data-ajax-popup="true" data-bs-toggle="{{__('Create Project Task Stage')}}">
                Create
            </a>
                                            ...
                                            <span class="float-end">
                                                @can('edit project task stage')
                                                    <div class="action-btn bg-info ms-2">
                                                        <a href="#" data-url="{{ URL::to('project-task-stages/'.$task_stage->id.'/edit') }}" data-ajax-popup="true"   data-bs-title="{{__('Edit')}}" data-bs-toggle="{{__('Edit Bug Status')}}" class="mx-3 btn btn-sm d-inline-flex align-items-center">
                                                ...
                                                @can('delete project task stage')
                                                        <div class="action-btn bg-danger ms-2">
                                                            {!! Form::open(['method' => 'DELETE', 'route' => ['project-task-stages.destroy', $task_stage->id],'id'=>'delete-form-'.$task_stage->id]) !!}
                                                              <a href="#!" class="mx-3 btn btn-sm align-items-center bs-pass-para"  data-bs-title="{{__('Delete')}}" data-bs-title="{{__('Delete')}}" data-confirm="Are You Sure?|This action can not be undone. Do you want to continue?" data-confirm-yes="document.getElementById('delete-form-{{$task_stage->id}}').submit();">
