...
@section('action-btn')
    <div class="float-end">
        @can('create expense')
            <a href="#" class="btn btn-primary btn-sm" data-url="{{ route('projects.expenses.create',$project->id) }}" data-ajax-popup="true"  data-bs-title="{{__('Create')}}" data-size="lg" data-bs-toggle="{{__('Create Expense')}}">
                <span class="btn-inner--icon">Create</span>
            </a>
        @endcan
        <a href="{{ route('projects.show',$project->id) }}" class="btn btn-primary btn-sm"  data-bs-title="{{__('Back')}}">
                                    ...
                                    <tr>
                                        <th scope="row">
                                            @if(!empty($expense->attachment))
                                                <a href="{{ asset(Storage::url($expense->attachment)) }}" class="btn btn-sm btn-primary btn-icon rounded-pill"  data-bs-title="{{__('Download')}}" download>
                                            ...
                                            <td class="text-end w-15">
                                                <div class="actions">
                                                    @can('edit expense')
                                                        ...
                                                        <a href="#" class="mx-3 btn btn-sm align-items-center" data-url="{{ route('projects.expenses.edit',[$project->id,$expense->id]) }}" data-ajax-popup="true" data-size="lg" data-bs-toggle="{{__('Edit ').$expense->name}}"  data-bs-title="{{__('Edit')}}" title="Edit">
                                                    ...
                                                    @can('delete expense')
                                                            <div class="action-btn bg-danger ms-2">
                                                                ...
                                                                <a href="#" class="mx-3 btn btn-sm align-items-center bs-pass-para"  data-bs-title="{{__('Delete')}}" data-bs-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?')}}|{{__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-expense-{{$expense->id}}').submit();">
