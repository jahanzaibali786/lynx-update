...
@section('page-title')
    {{ __('Employee Birthday Report') }}
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item">{{ __('Employee Birthday Report') }}</li>
@endsection
...
@push('script-page')
    ...
    <script>
        ...
        function printPdf() {
            const form = document.getElementById('empBirthdayRpt');
...
@section('content')
            ...
            <div class="mt-2" id="multiCollapseExample1">
                <div class="card">
                    <div class="card-body filter_change">
                        {{ Form::open(['route' => ['empBirthdayRpt'], 'method' => 'GET', 'id' => 'empBirthdayRpt']) }}
                        <div class="row d-flex justify-content-end">
                            ...
                            <div class="col-auto float-end ms-2 mt-4">
                                <a href="#" class="btn mx-1 btn-sm btn-outline-primary"
                                    onclick="document.getElementById('empBirthdayRpt').submit(); return false;"
                                    data-bs-title="{{ __('Apply') }}">
                                    <span class="btn-inner--icon">Search</span>
                                </a>
                                <a href="{{ route('empBirthdayRpt') }}" class="btn mx-1 btn-sm btn-outline-danger"
                ...
                <table class="datatable">
                    ...
                    <tbody>
                        @if (!empty($employeesquery) && $employeesquery->count())
                            ...
                            {{-- @php $serial = ($employeesquery->currentPage() - 1) * $employeesquery->perPage() + 1; @endphp --}}

                            @foreach ($groupedEmployees as $branchName => $employees)
                                <tr>
                                    <td colspan="5" class="font-weight-bold bg-light">{{ $branchName }}</td>
                                </tr>
    ...
    {{-- @if ($employeesquery->hasPages())
        <div class="pagination">
            <ul>
                @if ($employeesquery->onFirstPage())
                    <li class="disabled">&laquo; Previous</li>
                @else
                    <li><a href="{{ $employeesquery->appends(request()->query())->previousPageUrl() }}"
                            rel="prev">&laquo; Previous</a></li>
                @endif
                @if ($employeesquery->currentPage() > 1)
                    <li><a href="{{ $employeesquery->appends(request()->query())->url(1) }}">First</a></li>
                @endif
                @php
                    $currentPage = $employeesquery->currentPage();
                    $lastPage = $employeesquery->lastPage();
                ...
                @for ($page = $startPage; $page <= $endPage; $page++)
                    <li class="{{ $page == $employeesquery->currentPage() ? 'active' : '' }}">
                        <a href="{{ $employeesquery->appends(request()->query())->url($page) }}">{{ $page }}</a>
                    </li>
                @endfor
                @if ($employeesquery->hasMorePages())
                    <li><a href="{{ $employeesquery->appends(request()->query())->nextPageUrl() }}" rel="next">Next
                            &raquo;</a></li>
                @else
                    <li class="disabled">Next &raquo;</li>
                @endif
                @if ($employeesquery->currentPage() < $employeesquery->lastPage())
                    <li><a
                            href="{{ $employeesquery->appends(request()->query())->url($employeesquery->lastPage()) }}">Last</a>
                    </li>
                @endif
            </ul>
        </div>
