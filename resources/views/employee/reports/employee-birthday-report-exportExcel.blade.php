...
@endif
<tr>
        <td colspan="{{ $colspan ?? 5 }}" style="text-align: left; font-family: calibri; font-weight: bold; font-size: 15px; font-weight: bold;">
        <span style="text-transform: uppercase;">
...
<table>
    ...
    <tbody>
        @php
            // Normalize and guard
            $employeesquery = collect($data['employeesquery'] ?? []);
        @endphp

        @if($employeesquery->count())
            @php
                // Group by branch name (handles nulls safely)
                $groupedEmployees = $employeesquery->groupBy(function ($e) {
                    return optional($e->userbranch)->name ?? 'Unknown Branch';
                });

                // Global row index across all groups
                $rowIndex = 1;
            @endphp

            @foreach ($groupedEmployees as $branchName => $emps)
                <tr>
                    <td colspan="2" class="background-color: gray; text-align: left; font-weight: bold;">{{ $branchName }}</td>
                    <td colspan="3" class="background-color: gray; text-align: left; font-weight: bold;"></td>
                </tr>

                @foreach ($emps as $e)
                    <tr>
                        <td>{{ $rowIndex++ }}</td>
                        <td>{{ optional($e->userbranch)->name }}</td>
                        <td>{{ $e->employee_id }}</td>
                        <td>{{ $e->name }}</td>
                        <td>{{ \Carbon\Carbon::parse($e->dob)->format('d-M-Y') }}</td>
                    </tr>
                @endforeach
            @endforeach
        @else
