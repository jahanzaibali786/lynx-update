...
@section('page-title')
    {{ __('Emp. Monthly Reconcilation') }}
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item">{{ __('Employee Monthly Reconcilation') }}</li>
@endsection
...
@push('script-page')
    ...
    <script>
        function submitWithPrintFlag() {
            const form = document.getElementById('employeeMonthlyReconsilation');
                ...
                <div class="mt-2" id="multiCollapseExample1">
                    <div class="card">
                        <div class="card-body filter_change">
                            {{ Form::open(['route' => ['employeeMonthlyReconsilation'], 'method' => 'GET', 'id' => 'employeeMonthlyReconsilation']) }}
                            <div class="row d-flex justify-content-end">
                                ...
                                <div class="col-auto float-end ms-2 mt-4">
                                    <a href="#" class="btn mx-1 btn-sm btn-outline-primary"
                                        onclick="document.getElementById('employeeMonthlyReconsilation').submit(); return false;"
                                         data-bs-title="{{ __('Apply') }}">
                                        <span class="btn-inner--icon">Search</span>
                                    </a>
                                    <a href="{{ route('employeeMonthlyReconsilation') }}"
        ...
        <div class="card p-4">
            ...
            @if (request()->filled('month'))
                <p style="text-align:center; font-weight:600; font-size:1.1rem;">
                    Salary for the Month of {{ Carbon::createFromFormat('Y-m', request('month'))->format('F Y') }}
                </p>
            @else
                <p style="text-align:center; font-weight:600; font-size:1.1rem;">
                    Salary for the Month of {{ Carbon::now()->format('F Y') }}
                </p>
            @endif
                    ...
                    <tbody>
                        @if (!empty($data))
                            @if (request()->has('branches') && request()->branches != '')
                                @php
                                    $branch = \Auth::user()->getBranch(request()->branches);
                                    $totalGross = $totalNet = $totalDiff = $totalPerDay = 0;
                                    $totalDays = 0;
                                @endphp
                                <tr style="background-color: #cdcaca;">
                                    <td colspan="8" style="font-weight: bold; text-align: left;">{{ $branch->name }}
                                    </td>
                                </tr>
                                @foreach ($data as $empsalaries)
                                    @foreach ($empsalaries as $empsalary)
                                        @php
                                            $totalGross += $empsalary->gross;
                                            $totalNet += $empsalary->net_pay;
                                            $totalDiff += $empsalary->gross - $empsalary->net_pay;
                                            $totalPerDay += $empsalary->sal_days
                                                ? $empsalary->net_pay / $empsalary->sal_days
                                                : 0;
                                            $totalDays++;
                                        @endphp
                                        <tr>
                                            ...
                                            <td>{{ $empsalary->gross }}</td>
                                            <td>{{ $empsalary->net_pay }}</td>
                                            <td>{{ $empsalary->gross - $empsalary->net_pay }}</td>
                                            <td>{{ number_format($empsalary->net_pay / $empsalary->sal_days) }}</td>
                                        </tr>
                                    @endforeach
                                @endforeach
                                {{-- Branch total row --}}
                                ...
                                @foreach ($data as $branchId => $branchData)
                                    ...
                                    @foreach ($branchData as $empsalary)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $empsalary->gross - $empsalary->net_pay }}</td>
                                            @php
                                                if ($empsalary->sal_days == 0) {
                                                    $perDaySalary = 0;
                                                } else {
                                                    $perDaySalary = $empsalary->net_pay / $empsalary->sal_days;
                                                }
                                            @endphp
