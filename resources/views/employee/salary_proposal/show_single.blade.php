...
<div class="modal-body salary-proposal-single-show">
    <style>
        .salary-proposal-single-show{--sp-border:#e5e7eb;--sp-muted:#64748b;--sp-text:#1e293b;background:#f8fafc;padding:.85rem}
        .salary-proposal-single-show .sp-shell{display:flex;flex-direction:column;gap:.75rem}
        .salary-proposal-single-show .sp-panel{background:#fff;border:1px solid var(--sp-border);border-radius:12px;box-shadow:0 2px 8px rgba(15,23,42,.04);overflow:hidden}
        .salary-proposal-single-show .sp-head{padding:.65rem .8rem;border-bottom:1px solid var(--sp-border);display:flex;align-items:center;justify-content:space-between;gap:.6rem;background:#eef2ff}
        .salary-proposal-single-show .sp-head.info{background:#f0f9ff}.salary-proposal-single-show .sp-head.success{background:#ecfdf5}.salary-proposal-single-show .sp-head.warning{background:#fff7ed}
        .salary-proposal-single-show .sp-title{font-size:1rem;font-weight:800;color:var(--sp-text);margin:0}.salary-proposal-single-show .sp-sub{font-size:.76rem;color:var(--sp-muted);margin-top:.08rem}
        .salary-proposal-single-show .sp-badge{padding:.28rem .55rem;border-radius:999px;font-size:.76rem;font-weight:800;white-space:nowrap}.salary-proposal-single-show .sp-badge-warning{background:#ffedd5;color:#9a3412}.salary-proposal-single-show .sp-badge-success{background:#d1fae5;color:#065f46}.salary-proposal-single-show .sp-badge-danger{background:#fee2e2;color:#991b1b}
        .salary-proposal-single-show .sp-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr))}.salary-proposal-single-show .sp-item{padding:.7rem .8rem;border-right:1px solid var(--sp-border);border-bottom:1px solid var(--sp-border);min-height:66px}.salary-proposal-single-show .sp-item:nth-child(4n){border-right:0}
        .salary-proposal-single-show .sp-label{display:block;font-size:.7rem;font-weight:800;color:var(--sp-muted);text-transform:uppercase;letter-spacing:.035em;margin-bottom:.12rem}.salary-proposal-single-show .sp-value{font-size:.9rem;font-weight:700;color:var(--sp-text);word-break:break-word}
        .salary-proposal-single-show .sp-section-body{padding:.8rem}.salary-proposal-single-show .table{margin-bottom:0;font-size:.88rem}.salary-proposal-single-show .table th{font-size:.75rem;text-transform:uppercase;color:#475569;background:#f8fafc;white-space:nowrap}.salary-proposal-single-show .table td,.salary-proposal-single-show .table th{padding:.48rem .55rem;vertical-align:middle}.salary-proposal-single-show .sp-money{text-align:right;font-variant-numeric:tabular-nums;white-space:nowrap}
        .salary-proposal-single-show .sp-total-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:.5rem}.salary-proposal-single-show .sp-total{border:1px solid var(--sp-border);border-radius:9px;padding:.6rem;background:#fff}.salary-proposal-single-show .sp-total strong{display:block;font-size:1rem;margin-top:.15rem}
        .salary-proposal-single-show .sp-tabs{display:flex;gap:.35rem;padding:.55rem .6rem 0;background:#fff;border-bottom:1px solid var(--sp-border);flex-wrap:wrap}
        .salary-proposal-single-show .sp-tab-btn{border:0;background:transparent;color:var(--sp-muted);font-size:.86rem;font-weight:800;padding:.5rem .7rem;border-radius:8px 8px 0 0;cursor:pointer}
        .salary-proposal-single-show .sp-tab-btn.active[data-target="structure"]{background:#f0f9ff;color:#0369a1}
        .salary-proposal-single-show .sp-tab-btn.active[data-target="additions"]{background:#ecfdf5;color:#047857}
        .salary-proposal-single-show .sp-tab-btn.active[data-target="deductions"]{background:#fff7ed;color:#b45309}
        .salary-proposal-single-show .sp-tab-btn.active[data-target="accounting"]{background:#f8fafc;color:#334155;border:1px solid #cbd5e1;border-bottom:0}
        .salary-proposal-single-show .sp-tab-pane{display:none;background:#fff}
        .salary-proposal-single-show .sp-tab-pane.active{display:block}

        .salary-proposal-single-show .sp-action-panel{background:#fff;border:1px solid var(--sp-border);border-radius:10px;padding:.75rem .85rem;display:flex;align-items:center;justify-content:space-between;gap:.75rem;flex-wrap:wrap}
        .salary-proposal-single-show .sp-action-copy{min-width:220px}.salary-proposal-single-show .sp-action-title{font-size:.9rem;font-weight:800;color:var(--sp-text);margin:0}.salary-proposal-single-show .sp-action-sub{font-size:.76rem;color:var(--sp-muted);margin-top:.12rem}
        .salary-proposal-single-show .sp-action-buttons{display:flex;align-items:center;gap:.45rem;flex-wrap:wrap}.salary-proposal-single-show .sp-action-buttons form{margin:0}.salary-proposal-single-show .sp-action-buttons .btn{min-width:92px}
        .salary-proposal-single-show .sp-accounting-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.7rem;padding:.85rem}.salary-proposal-single-show .sp-accounting-field label{display:block;font-size:.74rem;font-weight:800;color:#475569;margin-bottom:.3rem}.salary-proposal-single-show .sp-accounting-field select{width:100%;min-height:36px}.salary-proposal-single-show .sp-accounting-note{padding:.55rem .75rem;margin:.8rem .85rem 0;border:1px solid #dbe3ec;background:#f8fafc;border-radius:8px;font-size:.78rem;color:#475569}
        @media(max-width:900px){.salary-proposal-single-show .sp-grid,.salary-proposal-single-show .sp-total-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.salary-proposal-single-show .sp-item:nth-child(4n){border-right:1px solid var(--sp-border)}.salary-proposal-single-show .sp-item:nth-child(2n){border-right:0}}
        @media(max-width:575px){.salary-proposal-single-show .sp-grid,.salary-proposal-single-show .sp-total-grid,.salary-proposal-single-show .sp-accounting-grid{grid-template-columns:1fr}.salary-proposal-single-show .sp-item{border-right:0!important}}
    </style>
                ...
                <div class="sp-tab-pane" data-pane="accounting">
                    <div class="sp-head">
                        <div>
                            ...
                            <div class="sp-sub">These mappings will be applied when this single salary proposal is approved.</div>
                        </div>
                    </div>
        ...
        @if(Auth::user()->type === 'company')
            <div class="sp-action-panel">
                <div class="sp-action-copy">
                    ...
                    <div class="sp-action-sub">Review this single salary proposal and approve or reject it.</div>
                </div>

                <div class="sp-action-buttons">
                    @if((int)($salaryProposal->status ?? 0) === 1)
                        <form action="{{ route('employee-salary-proporal.rollback', $salaryProposal->id) }}" method="POST" class="sp-single-action-form" data-action="reject">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-outline-danger">Reject</button>
                        </form>

                        <form id="single-proposal-approval-form" action="{{ route('employee-salary-proporal.approve', $salaryProposal->id) }}" method="POST" class="sp-single-action-form" data-action="approve">
                            @csrf
                        </form>
...
(function($){
    'use strict';
    const root = '.salary-proposal-single-show';
    $(document)
    ...
    $(document)
        .off('submit.salaryProposalSingleAction', root + ' .sp-single-action-form')
        .on('submit.salaryProposalSingleAction', root + ' .sp-single-action-form', function(e){
