...
@section('action-btn')
    <div class="float-end">
        <a href="{{ route('zoom-meeting.calender') }}" class="btn btn-sm btn-primary"  data-bs-title="{{__('Calender View')}}" data-bs-title="{{__('Calender View')}}">
            <i class="ti ti-calendar"></i>
        </a>
        <a href="#" data-size="lg" data-url="{{ route('zoom-meeting.create') }}" data-ajax-popup="true"  data-bs-title="{{__('Create')}}" data-bs-toggle="{{__('Create  New Meeting')}}" class="btn btn-sm btn-primary">
            Create
        </a>
    </div>
@endsection
                                        ...
                                        <div class="avatar-group">
                                            @foreach($item->users($item->user_id) as $projectUser)
                                                <img alt="image"  data-bs-placement="top"
                                                     data-bs-title="{{$projectUser->name}}"
                                    ...
                                    @if(\Auth::user()->type == 'company')
                                        <td class="text-end">
                                            <div class="action-btn bg-danger ms-2">
                                                ...
                                                <a href="#" class="mx-3 btn btn-sm  align-items-center bs-pass-para"  data-bs-title="{{__('Delete')}}" data-bs-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$item->id}}').submit();"><i class="ti ti-trash text-white"></i></a>
                                                {!! Form::close() !!}
                                            </div>
                                        </td>
                                    @endif
