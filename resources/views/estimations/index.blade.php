    ...
    <div class="all-button-box row d-flex justify-content-end">
        @can('create estimation')
            <div class="col-xl-2 col-lg-2 col-md-4 col-sm-6 col-6">
                <a href="#" data-url="{{ route('estimations.create') }}" data-size="sm" data-ajax-popup="true" data-bs-toggle="{{__('Create Estimate')}}" class="btn btn-xs btn-white btn-icon-only width-auto">Create {{__('Create')}}</a>
            </div>
        @endcan
    </div>
@endsection
                                    ...
                                    @if(Auth::user()->type != 'client')
                                        <td class="Action">
                                            ...
                                            @can('view estimation')
                                                ...
                                                @endcan
                                                @can('edit estimation')
                                                    <a href="#" data-url="{{ URL::to('estimations/'.$estimate->id.'/edit') }}" data-ajax-popup="true" data-bs-toggle="{{__('Edit Estimation')}}" class="edit-icon"  data-bs-title="{{__('Edit')}}"><i class="ti ti-pencil text-white"></i></a>
                                                @endcan
                                                @can('delete estimation')
                                                    <a href="#" class="delete-icon"  data-bs-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{$estimate->id}}').submit();"><i class="ti ti-trash"></i></a>
