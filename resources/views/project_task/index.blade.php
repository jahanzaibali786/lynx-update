                        ...
                        <div class="card">
                            <div class="card-header">
                                @can('create project task')
                                    <div class="float-end">
                                        <a href="#" data-size="lg" data-url="{{ route('projects.tasks.create',[$project->id,$stage->id]) }}" data-ajax-popup="true"  data-bs-title="{{__('Add Task in ').$stage->name}}" class="btn btn-sm btn-primary">
                                            Create
                                        </a>
                                    </div>
                                @endcan
                                ...
                                @foreach($tasks as $taskDetail)
                                    <div class="card draggable-item" id="{{$taskDetail->id}}">
                                        ...
                                        <div class="card-header border-0 pb-0 position-relative">
                                            <h5>
                                                <a href="#" data-url="{{ route('projects.tasks.show',[$project->id,$taskDetail->id]) }}" data-ajax-popup="true" data-size="lg" data-bs-title="{{$taskDetail->name}}">{{$taskDetail->name}}</a>
                                            </h5>
                                                ...
                                                <div class="btn-group card-option">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        ...
                                                        @can('view project task')
                                                            <a href="#!" data-size="md" data-url="{{ route('projects.tasks.show',[$project->id,$taskDetail->id]) }}" data-ajax-popup="true" class="dropdown-item" data-bs-title="{{__('View')}}">
                                                                <i class="ti ti-bookmark"></i>
                                                                <span>{{__('View')}}</span>
                                                            </a>
                                                        @endcan
                                                        @can('edit project task')
                                                            <a href="#!" data-size="lg" data-url="{{ route('projects.tasks.edit',[$project->id,$taskDetail->id]) }}" data-ajax-popup="true" class="dropdown-item" data-bs-title="{{__('Edit ').$taskDetail->name}}">
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex align-items-center justify-content-between mb-2">
                                                <ul class="list-inline mb-0">
                                                    <li class="list-inline-item d-inline-flex align-items-center"  data-bs-title="{{__('Files')}}">
                                                        <i class="f-16 text-primary ti ti-file"></i> {{ count($taskDetail->taskFiles) }}
                                                    </li>
                                                    <li class="list-inline-item d-inline-flex align-items-center"  data-bs-title="{{__('Task Progress')}}">
                                                        @if(str_replace('%','',$taskDetail->taskProgress()['percentage']) > 0)<span class="text-md">{{ $taskDetail->taskProgress()['percentage'] }}</span>@endif
                                                    </li>
                                                </ul>
                                                <div class="user-group">
                                                    @if(!empty($taskDetail->end_date) && $taskDetail->end_date != '0000-00-00')<span  data-bs-title="{{__('End Date')}}" @if(strtotime($taskDetail->end_date) < time())class="text-danger"@endif>{{ Utility::getDateFormated($taskDetail->end_date) }}</span>@endif
                                                </div>
                                            </div>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <ul class="list-inline mb-0">

                                                    <li class="list-inline-item d-inline-flex align-items-center"  data-bs-title="{{__('Comments')}}">
                                                        <i class="f-16 text-primary ti ti-message"></i> {{ count($taskDetail->comments) }}
                                                    </li>

                                                    <li class="list-inline-item d-inline-flex align-items-center"  data-bs-title="{{__('Task Checklist')}}">
                                                        <i class="f-16 text-primary ti ti-list"></i>{{ $taskDetail->countTaskChecklist() }}
                                                    </li>
                                                </ul>
                                                <div class="user-group">
                                                    @foreach($taskDetail->users() as $user)
                                                        <img @if($user->avatar) src="{{asset('/storage/uploads/avatar/'.$user->avatar)}}" @else src="{{asset('/storage/uploads/avatar/avatar.png')}}" @endif alt="image"  data-bs-title="{{(!empty($user)?$user->name:'')}}">
                                                    @endforeach
                                                </div>
                                            </div>
                                        </div>
