                                    ...
                                    <td>
                                        <div class="avatar-group">
                                            @if($task->users()->count() > 0)
                                                    ...
                                                    @foreach($users as $key => $user)
                                                        @if($key<3)
                                                            <a href="#" class="avatar rounded-circle avatar-sm">
                                                                <img data-bs-title="{{(!empty($user)?$user->name:'')}}" @if($user->avatar) src="{{asset('/storage/uploads/avatar/'.$user->avatar)}}" @else src="{{asset('/storage/uploads/avatar/avatar.png')}}" @endif data-bs-title="{{ $user->name }}" class="hweb">
                                                            </a>
                                                        @else
                                                            @break
                                                        @endif
                                                ...
                                                @if(count($users) > 3)
                                                        <a href="#" class="avatar rounded-circle avatar-sm">
                                                            <img  data-bs-title="{{(!empty($user)?$user->name:'')}}" @if($user->avatar) src="{{asset('/storage/uploads/avatar/'.$user->avatar)}}" @else src="{{asset('/storage/uploads/avatar/avatar.png')}}" @endif class="hweb">
                                                        </a>
                                                    @endif
                                                @else
                                                    {{ __('-') }}
                                        ...
                                        <td class="text-end w-15">
                                            <div class="actions">
                                                ...
                                                <a class="action-item px-1"  data-bs-title="{{__('Checklist')}}" data-bs-title="{{__('Checklist')}}">
