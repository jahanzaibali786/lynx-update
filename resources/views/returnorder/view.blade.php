            ...
            $(document).on('click', '#approve', function() {
                ...
                checkboxes.forEach(function(checkbox) {
                        if (checkbox.checked) {
                            ...
                            // if (cell.classList.contains('product_id')) {
                            //     product_id = cellContent;
                            // }
                        ...
                        // rowData.push({ product_id: product_id, });
                ...
                var order = $('#order').val();
                $.ajax({
                url: '{{ route('returnorder_approved') }}',
                type: 'POST',
                headers: {
                ...
                success: function (data) {
                    ...
                    // if(data.success == 'true'){
                        show_toastr('success', 'Status Update', 'success');
                    // }
                }
                });
            });
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('returnorder.index') }}">{{ __('Return Order Summary') }}</a></li>
    <li class="breadcrumb-item">{{ AUth::user()->quotationNumberFormat($quotation->quotation_id) }}</li>
@endsection

{{-- @section('action-btn')
    <div class="float-end">
        <a href="{{ route('returnorder.pdf', Crypt::encrypt($quotation->id))}}" class="btn btn-primary" target="_blank">{{__('Download')}}</a>
    </div>
@endsection --}}
                ...
                <div class="card-body">
                    <div class="row mt-2">
                        ...
                        <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12 text-end">
                            <h4 class="invoice-number">{{ Auth::user()->quotationNumberFormat($quotation->quotation_id) }}
                            </h4>
                        </div>
                        <div class="col-12">
                            <hr>
                    ...
                    <div class="row">
                        <div class="col-5">
                            <small class="font-style">
                                ...
                                {{ !empty($quotation->customer) ? $quotation->customer->name : '' }}<br>
                        ...
                        <div class="col-4">
                            ...
                            <small>
                                ...
                                {{ !empty($quotation->warehouse) ? $quotation->warehouse->name : '' }}<br>
                    ...
                    <div class="row mt-4">
                        <div class="col-md-12">
                            <div class="table-responsive mt-3">
                                <input type="hidden" name="order_id" id="order" value="{{ $quotation->id }}">
                                <table class="table">
                                    <thead>
                                        <tr>
                                    ...
                                    @foreach ($iteams as $key => $iteam)
                                        ...
                                        @endif
                                        <tr>
                                            <td>{{ $key + 1 }}  <input type="hidden" name="product_id" value="{{ $iteam->id }}"></td>
                                            ...
                                            <td> @if($iteam->status == 'approved') @else <input type="checkbox" name="checked[]" > @endif</td>
                                        </tr>
                                        @php
                                            $subTotal += $iteam->price * $iteam->quantity;
                                            $total = $subTotal;
                                        @endphp
                                    @endforeach
                                ...
                                <button class="btn btn-outline-primary" style="float: right;" id="approve">Approve</button>
                            </div>
                        </div>
                    </div>
                </div>
