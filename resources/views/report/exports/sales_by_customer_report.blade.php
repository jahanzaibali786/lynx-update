...
@endif
<tr>
        <td colspan="5" style="text-align: left; font-family: calibri; font-weight: bold; font-size: 14px; font-weight: bold;">
        <span style="text-transform: uppercase;">
...
@if(@$is_period)
<tr>
    <td colspan="5" style="text-align: left; font-family: calibri; font-weight: bold; font-size: 8px;">
        @php
...
<table>
    ...
    <tbody>
        @foreach ($data['invoices'] as $customer_id => $customer_invoices)
            @foreach ($customer_invoices as $invoice)
                <tr>
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $data['branches'][$customer_id] ?? 'Unknown Customer' }}</td>
