...
@section('content')
    ...
    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12">
                            <div class="form-group">
                                ...
                                {{ Form::select('store_to', $store_to, request('store_to'), ['class' => 'form-control select custom-select', 'required' => 'required']) }}
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="text-xs mt-1">
                                    @can('convert demand order to invoice')
                                        <a href="#" id="link-demand-order-btn" data-ajax-popup-over="true" data-url="{{ route('stock-transfer-note.draft_demand_orders') }}" data-title="{{ __('Link Demand Order') }}">{{ __('Link Demand Order') }}</a>
                                    @endcan
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal-footer">
            <input type="button" value="{{ __('Cancel') }}" onclick="location.href='{{ route('stock-transfer-note.index') }}'" class="btn btn-light">
            <input type="submit" value="{{ __('Create Stock Transfer Note') }}" class="btn btn-outline-primary">
        </div>
        {{ Form::close() }}
    </div>
    ...
    <script>
        (function() {
            ...
            function rebuildStudyPackOptions() {
                ...
                var currentValue = $('#study_pack_picker').val();
                ...
                initializeCustomSelects($('#invoice-form'));

                if (currentValue) {
                    $('#study_pack_picker').val(currentValue);
                }
            }
            ...
            function renderItemRow(entry, index, hidden) {
                ...
                var html = '<tr class="stn-item-row"' + (hidden ? ' style="display:none;"' : '') + ' data-entry-index="' + index + '">' +
                    '<td>' +
            ...
            function updateDemandOrderUrl() {
                ...
                var baseUrl = @json(route('stock-transfer-note.draft_demand_orders'));
                $('#link-demand-order-btn').attr('data-url', baseUrl + '?store_to=' + storeTo).data('url', baseUrl + '?store_to=' + storeTo);
            }
            ...
            $(document).on('click', '.stn-remove-item', function() {
                removeEntryByIndex(parseInt($(this).data('index'), 10));
            });

            $(document).on('input change', '.stn-quantity, .stn-price, .stn-item-description, .stn-type', function() {
            ...
            $(document).on('change', '.stn-item-select', function() {
                var index = parseInt($(this).data('index'), 10);
                var entry = state.entries[index];
                    ...
                    ajaxModalForm({
                        ...
                        onSuccess: function(response) {
                            if (response && response.success) {
                                ...
                                if (response.row_html && $('#stock-transfer-note-table-body').length) {
                                    $('#stock-transfer-note-table-body').prepend(response.row_html);
                                    if (typeof renumberStockTransferNoteRows === 'function') {
                                        renumberStockTransferNoteRows();
                                    }
                                    if (typeof initStockTransferNoteTooltips === 'function') {
                                        initStockTransferNoteTooltips(document.getElementById('stock-transfer-note-table-body'));
                                    }
                                } else if (typeof triggerContentAreaRefresh === 'function') {
