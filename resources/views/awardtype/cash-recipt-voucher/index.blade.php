...
@section('page-title')
    {{__('Manage Cash Recipt Voucher Entry')}}
@endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
    <li class="breadcrumb-item">{{__('Vouchers')}}</li>
    <li class="breadcrumb-item">{{__('Cash Recipt Voucher Entry')}}</li>
@endsection

@section('action-btn')
    <div class="float-end">
        @can('create journal entry')
            <a href="{{ route('cash-recipt-voucher.create') }}" data-bs-toggle="{{__('Create New Cash Recipt Voucher')}}"   data-bs-title="{{__('Create')}}" class="btn mx-1 btn-sm btn-outline-primary">
                        ...
                        <div class="row d-flex justify-content-end ">
                            ...
                            <div class="col-auto float-end ms-2 mt-4">
                                <a href="#" class="btn mx-1 btn-sm btn-outline-primary"
                                    onclick="document.getElementById('cash-recipt-voucher_submit').submit(); return false;"
                                     data-bs-title="{{ __('apply') }}">
                                    <span class="btn-inner--icon">Search</span>
                                </a>
                                <a href="{{ route('cash-recipt-voucher.index') }}" class="btn mx-1 btn-sm btn-outline-danger" 
    ...
    @endif
                        <table class="">
                            ...
                            @foreach ($journalEntries as $journalEntry)
                                <tr>
                                    <td class="Id">
                                        <a href="{{ route('cash-recipt-voucher.show',$journalEntry->id) }}" class="btn btn-outline-primary">{{ AUth::user()->CRVNumberFormat($journalEntry->journal_id) }}</a>
                                    </td>
                                    ...
                                    <div class="action-btn  ms-2">
                                        @can('edit journal entry')
                                                ...
                                                <a data-bs-toggle="{{__('Edit Cash Recipt Voucher')}}" href="{{ route('cash-recipt-voucher.edit',[$journalEntry->id]) }}" class="mx-1 btn mx-1 btn-sm btn-outline-primary align-items-center"  data-bs-title="{{__('Edit')}}" data-bs-title="{{__('Edit')}}">
                                        ...
                                        @can('delete journal entry')
                                                    ...
                                                    {!! Form::open(['method' => 'DELETE', 'route' => array('cash-recipt-voucher.destroy', $journalEntry->id),'id'=>'delete-form-'.$journalEntry->id]) !!}
