        ...
        @can('create plan')
            @if(isset($admin_payment_setting) && !empty($admin_payment_setting))
                ...
                || $admin_payment_setting['is_aamarpay_enabled'] == 'on' || $admin_payment_setting['is_paytr_enabled'] == 'on')
                    <a href="#" data-size="lg" data-url="{{ route('plans.create') }}" data-ajax-popup="true"  data-bs-title="{{__('Create')}}" data-bs-toggle="{{__('Create New Plan')}}" class="btn btn-sm btn-primary">
                        Create
                    </a>
                @endif
            @endif
                   ...
                   ">
                    <div class="card-body">
                        ...
                        @if(\Auth::user()->type =='super admin')
                            <div class="col-4">
                                <a data-bs-title="{{__('Edit Plan')}}" href="#" class="btn btn-primary btn-icon m-1" data-url="{{ route('plans.edit',$plan->id) }}" data-ajax-popup="true" data-bs-toggle="{{__('Edit Plan')}}" data-size="lg"  data-bs-title="{{__('Edit')}}">
                                ...
                                @if(\Auth::user()->type != 'super admin')
                                    ...
                                    @if($plan->id != 1 && $plan->id != \Auth::user()->plan)
                                        @if(\Auth::user()->requested_plan != $plan->id)
                                            <a href="{{ route('send.request',[\Illuminate\Support\Facades\Crypt::encrypt($plan->id)])}}" class="btn btn-primary btn-icon m-1" data-bs-toggle="{{__('Send Request')}}"  data-bs-title="{{__('Send Request')}}">
                                                <span class="btn-inner--icon"><i class="ti ti-corner-up-right"></i></span>
                                            </a>
                                        @else
                                            <a href="{{ route('request.cancel',\Auth::user()->id) }}" class="btn btn-danger btn-icon m-1" data-bs-toggle="{{__('`Cancle Request')}}"  data-bs-title="{{__('Cancle Request')}}">
