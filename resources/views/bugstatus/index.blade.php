...
@section('action-btn')
    @can('create bug status')
        <div class="float-end">
            <a href="#" data-url="{{ route('bugstatus.create') }}"   data-bs-title="{{__('Create')}}" class="btn btn-sm btn-primary" data-ajax-popup="true" data-bs-toggle="{{__('Create Bug Stage')}}">
                Create
            </a>
        </div>
    @endcan
                                            ...
                                            <span class="float-end">
                                                @can('edit bug status')
                                                    <div class="action-btn bg-info ms-2">
                                                        <a href="#" data-url="{{ URL::to('bugstatus/'.$bug->id.'/edit') }}" data-ajax-popup="true"   data-bs-title="{{__('Edit')}}" data-bs-toggle="{{__('Edit Bug Status')}}" class="mx-3 btn btn-sm d-inline-flex align-items-center">
                                                ...
                                                @can('delete bug status')
                                                    <div class="action-btn bg-danger ms-2">
                                                        {!! Form::open(['method' => 'DELETE', 'route' => ['bugstatus.destroy', $bug->id],'id'=>'delete-form-'.$bug->id]) !!}
                                                              <a href="#!" class="mx-3 btn btn-sm align-items-center bs-pass-para"  data-bs-title="{{__('Delete')}}" data-bs-title="{{__('Delete')}}" data-confirm="Are You Sure?|This action can not be undone. Do you want to continue?" data-confirm-yes="document.getElementById('delete-form-{{$bug->id}}').submit();">
