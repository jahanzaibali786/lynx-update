...
@section('action-btn')
    <div class="float-end">
        ...
        @if($view == 'grid')
            <a href="{{ route('bugs.view', 'list') }}" class="btn btn-primary btn-sm"  data-bs-title="{{__('List View')}}">
                <span class="btn-inner--text"><i class="ti ti-list"></i></span>
            </a>
        @else
            <a href="{{ route('bugs.view', 'grid') }}" class="btn btn-primary btn-sm"  data-bs-title="{{__('Card View')}}">
        ...
        @can('manage project')
            ...
            <a href="{{ route('projects.index') }}" class="btn btn-primary btn-sm"  data-bs-title="{{__('Back')}}">
                                ...
                                @foreach($bugs as $bug)
                                    ...
                                    <span class="me-5 badge p-2 px-3 rounded bg-{{ (\Auth::user()->checkProject($bug->project_id) == 'Owner') ? 'success' : 'warning'  }}">{{ __(\Auth::user()->checkProject($bug->project_id)) }}</span>
                                            ...
                                            <div class="avatar-group">
                                                @if($bug->users()->count() > 0)
                                                    ...
                                                    <a href="#" class="avatar rounded-circle avatar-sm">
                                                        <img data-bs-title="{{(!empty($user[0])?$user[0]->name:'')}}" @if($user[0]->avatar) src="{{asset('/storage/uploads/avatar/'.$user[0]->avatar)}}" @else src="{{asset('/storage/uploads/avatar/avatar.png')}}" @endif data-bs-title="{{ $user[0]->name }}" class="hweb">
                                                    </a>
                                                    @if($users = $bug->users())
                                        ...
                                        <td class="text-end w-15">
                                            <div class="actions">
                                                <a class="action-item px-1"  data-bs-title="{{__('Attachment')}}" data-bs-title="{{__('Attachment')}}">
                                                    <i class="ti ti-paperclip mr-2"></i>{{ count($bug->bugFiles) }}
                                                </a>
                                                <a class="action-item px-1"  data-bs-title="{{__('Comment')}}" data-bs-title="{{__('Comment')}}">
                                                    <i class="ti ti-brand-hipchat mr-2"></i>{{ count($bug->comments) }}
                                                </a>
                                                <a class="action-item px-1"  data-bs-title="{{__('Checklist')}}">
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
