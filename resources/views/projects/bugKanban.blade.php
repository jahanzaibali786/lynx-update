...
@section('action-btn')
    <div class="float-end">
        @can('manage bug report')
            <a href="{{ route('task.bug',$project->id) }}"  data-bs-title="{{__('List')}}" class="btn btn-sm btn-primary">
                <i class="ti ti-list"></i>
            </a>
        @endcan
        @can('create bug report')
            <a href="#" data-size="lg" data-url="{{ route('task.bug.create',$project->id) }}" data-ajax-popup="true"  data-bs-title="{{__('Create New Bug')}}" class="btn btn-sm btn-primary">
                Create
            </a>
        @endcan
    </div>
                                ...
                                @foreach($bugs as $bug)
                                    <div class="card draggable-item" id="{{$bug->id}}">
                                        ...
                                        <div class="card-header border-0 pb-0 position-relative">
                                            <h5>
                                                <a href="#" data-url="{{ route('task.bug.show',[$project->id,$bug->id]) }}" data-ajax-popup="true" data-size="lg" data-bs-title="{{$bug->title}}">{{$bug->title}}</a>
                                            </h5>
                                                    ...
                                                    @if(Gate::check('edit bug report') || Gate::check('delete bug report'))
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            @can('edit project task')
                                                                <a href="#!" data-size="lg" data-url="{{ route('task.bug.edit',[$project->id,$bug->id]) }}" data-ajax-popup="true" class="dropdown-item" data-bs-title="{{__('Edit ').$bug->name}}">
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex align-items-center justify-content-between mb-2">
                                                <ul class="list-inline mb-0">
                                                    <li class="list-inline-item d-inline-flex align-items-center"  data-bs-title="{{__('Start Date')}}">
                                                         {{ \Auth::user()->dateFormat($bug->start_date) }}
                                                    </li>

                                                </ul>
                                                <div class="user-group">
                                                    <span  data-bs-title="{{__('End Date')}}">  {{ \Auth::user()->dateFormat($bug->due_date) }}</span>
                                                </div>
                                            </div>
                                            <div class="d-flex align-items-center justify-content-between">
                                                ...
                                                <div class="user-group">
                                                    <img @if(isset($user[0]->avatar)) src="{{asset('/storage/uploads/avatar/'.$user[0]->avatar)}}" @else src="{{asset('/storage/uploads/avatar/avatar.png')}}" @endif alt="image"  data-bs-title="{{(!empty($user[0])?$user[0]->name:'')}}">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
