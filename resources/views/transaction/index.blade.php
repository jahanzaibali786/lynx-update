...
@section('action-btn')
    <div class="float-end">
        ...
        <a href="{{route('transaction.export')}}"  data-bs-title="{{__('Export')}}" class="btn mx-1 btn-sm btn-outline-primary">
            <span class="btn-inner--icon">Export</span>
        </a>

        <a href="#" class="btn mx-1 btn-sm btn-outline-primary" onclick="saveAsPDF()" data-bs-title="{{__('Download')}}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row align-items-center justify-content-end">
                            ...
                            <div class="col-auto mt-4">
                                <div class="row">
                                    <div class="col-auto">
                                        <a href="#" class="btn mx-1 btn-sm btn-outline-primary" onclick="document.getElementById('transaction_report').submit(); return false;"  data-bs-title="{{__('apply')}}">
                                            <span class="btn-inner--icon">Search</span></a>

                                        <a href="{{route('transaction.index')}}" class="btn mx-1 btn-sm btn-outline-danger "  data-bs-title="{{__('Reset')}}">
    ...
    <div class="row">
        ...
        <div class="pagination">
            <ul>
                ...
                @else
                    <li><a href="{{ $transactions->appends(request()->query())->previousPageUrl() }}"
                            rel="prev">&laquo; Previous</a></li>
                @endif
                @if ($transactions->currentPage() > 1)
                    <li><a href="{{ $transactions->appends(request()->query())->url(1) }}">First</a></li>
                @endif
                @php
                ...
                @for ($page = $startPage; $page <= $endPage; $page++)
                    <li class="{{ $page == $transactions->currentPage() ? 'active' : '' }}">
                        <a href="{{ $transactions->appends(request()->query())->url($page) }}">{{ $page }}</a>
                    </li>
                @endfor
                @if ($transactions->hasMorePages())
                    <li><a href="{{ $transactions->appends(request()->query())->nextPageUrl() }}" rel="next">Next
                            &raquo;</a></li>
                @else
                ...
                @if ($transactions->currentPage() < $transactions->lastPage())
                    <li><a
                            href="{{ $transactions->appends(request()->query())->url($transactions->lastPage()) }}">Last</a>
                    </li>
                @endif
            </ul>
        </div>
