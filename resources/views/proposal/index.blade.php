...
<div class="float-end">
    ...
    <a href="{{route('proposal.export')}}" class="btn btn-sm btn-primary" 
        data-bs-title="{{__('Export')}}">
        Export
    </a>

    @can('create proposal')
    <a href="{{ route('proposal.create',0) }}" class="btn btn-sm btn-primary" 
        ...
        <tr class="font-style">
            ...
            <td class="Action">
                ...
                <div class="action-btn bg-warning ms-2">
                    {!! Form::open(['method' => 'get', 'route' => ['proposal.convert',
                    $proposal->id],'id'=>'proposal-form-'.$proposal->id]) !!}

                    <a href="#" class="mx-3 btn btn-sm align-items-center bs-pass-para" 
                        data-bs-title="{{__('Convert Invoice')}}" data-bs-title="{{__('Convert to Invoice')}}"
                ...
                <div class="action-btn bg-warning ms-2">
                    <a href="{{ route('invoice.show',\Crypt::encrypt($proposal->converted_invoice_id)) }}"
                        ...
                        data-bs-title="{{__('Already convert to Invoice')}}"
                        data-bs-title="{{__('Already convert to Invoice')}}">
                ...
                <div class="action-btn bg-success ms-2">
                    {!! Form::open(['method' => 'get', 'route' => ['proposal.duplicate',
                    $proposal->id],'id'=>'duplicate-form-'.$proposal->id]) !!}

                    <a href="#" class="mx-3 btn btn-sm  align-items-center bs-pass-para" 
                        data-bs-title="{{__('Duplicate')}}" data-bs-title="{{__('Duplicate')}}"
                        data-bs-title="{{__('Delete')}}"
                ...
                <div class="action-btn bg-info ms-2">
                    <a href="{{ route('proposal.show',\Crypt::encrypt($proposal->id)) }}"
                        class="mx-3 btn btn-sm  align-items-center"  data-bs-title="{{__('Show')}}"
                ...
                <div class="action-btn bg-danger ms-2">
                    {!! Form::open(['method' => 'DELETE', 'route' => ['proposal.destroy',
                    $proposal->id],'id'=>'delete-form-'.$proposal->id]) !!}
...
<div class="pagination">
    <ul>
        ...
        @else
        <li><a href="{{ $proposals->appends(request()->query())->previousPageUrl() }}" rel="prev">&laquo;</a></li>
        @endif
        @for ($page = 1; $page <= $proposals->lastPage(); $page++)
            <li class="{{ $page == $proposals->currentPage() ? 'active' : '' }}">
                <a href="{{ $proposals->appends(request()->query())->url($page) }}">{{ $page }}</a>
            </li>
            @endfor
            @if ($proposals->hasMorePages())
            <li><a href="{{ $proposals->appends(request()->query())->nextPageUrl() }}" rel="next">&raquo;</a></li>
            @else
