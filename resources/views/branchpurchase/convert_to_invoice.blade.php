...
function appendInlineRow() {
    ...
    var row = '<tr data-row-id="' + rid + '" class="inline-edit-row">' +
        '<td class="col-item">' +
        '<select class="form-control form-control-sm custom-select row-item" data-rid="' + rid + '">' + PRODUCT_OPTS + '</select>' +
        ...
        '</td>' +
        '<td class="col-quantity"><input type="number" class="form-control form-control-sm row-quantity" data-rid="' + rid + '" min="1" step="1" value="1"><span class="unit-label" id="unit-' + rid + '"></span></td>' +
        '<td class="col-price"><input type="number" class="form-control form-control-sm row-price" data-rid="' + rid + '" min="0" step="0.01"></td>' +
        '<td class="col-discount"><input type="number" class="form-control form-control-sm row-discount" data-rid="' + rid + '" min="0" step="0.01" value="0"></td>' +
        '<td class="col-type"><select class="form-control form-control-sm custom-select row-type" data-rid="' + rid + '"><option value="new">New</option><option value="use">Used</option><option value="damage">Damage</option></select></td>' +
        '<td class="col-amount text-end"><span class="amount-display" id="amount-' + rid + '">0.00</span></td>' +
        '<td class="col-actions text-center"><button type="button" class="btn btn-sm btn-primary confirm-row-btn" data-rid="' + rid + '" title="Confirm"><i class="ti ti-check"></i></button> <button type="button" class="btn btn-sm btn-outline-danger discard-row-btn" data-rid="' + rid + '" title="Discard"><i class="ti ti-x"></i></button></td>' +
        '</tr>';

    $('#items-tbody').append(row);
    $('#empty-row').hide();
    setTimeout(function() { $('.row-item[data-rid="' + rid + '"]').focus(); }, 100);
}

$(document).on('change', '.row-item', function() {
    var rid = $(this).data('rid');
    var itemId = $(this).val();
    if (!itemId) { clearItemData(rid); return; }
    ...
    $.ajax({
        ...
        error: function() { notify('error', 'Failed to load product details.'); }
    });
});
...
$(document).on('click', '.confirm-row-btn', function() {
    ...
    if (!itemId) { notify('error', 'Please select an item.'); return; }
    if (quantity <= 0) { notify('error', 'Please enter a valid quantity.'); return; }
    if (price <= 0) { notify('error', 'Please enter a valid price.'); return; }
...
function buildLockedRow(e) {
    ...
    return '<tr data-entry-id="' + e.id + '" class="confirmed-row">' +
        ...
        '<td class="text-center"><a href="#" class="edit-entry-btn text-primary me-1" data-id="' + e.id + '" title="Edit"><i class="ti ti-pencil"></i></a><a href="#" class="remove-entry-btn text-danger" data-id="' + e.id + '" title="Remove"><i class="ti ti-trash"></i></a></td>' +
        '</tr>';
}
...
$(document).on('click', '.remove-entry-btn', function(e) {
    ...
    invoiceItems = invoiceItems.filter(function(item) { return parseInt(item.id) !== id; });
...
$(document).on('click', '.edit-entry-btn', function(e) {
    e.preventDefault();
    var id = parseInt($(this).data('id'));
    var entry = invoiceItems.find(function(item) { return parseInt(item.id) === id; });
    if (!entry) return;
    ...
    tr.find('td').eq(2).html('<input type="number" class="form-control form-control-sm edit-price" value="' + entry.price + '" min="0" step="0.01" data-id="' + id + '" style="width:100px;">');
    tr.find('td').eq(3).html('<input type="number" class="form-control form-control-sm edit-discount" value="' + entry.discount + '" min="0" step="0.01" data-id="' + id + '" style="width:100px;">');
    tr.find('td').eq(4).html('<select class="form-control form-control-sm edit-type" data-id="' + id + '"><option value="new" ' + (entry.type === 'new' ? 'selected' : '') + '>New</option><option value="use" ' + (entry.type === 'use' ? 'selected' : '') + '>Used</option><option value="damage" ' + (entry.type === 'damage' ? 'selected' : '') + '>Damage</option></select>');
    tr.find('td').eq(5).html('<span class="edit-amount text-end fw-semibold">' + formatAmount(entry.amount) + '</span>');
    tr.find('td').eq(6).html('<a href="#" class="save-edit-btn text-success me-1" data-id="' + id + '" title="Save"><i class="ti ti-check"></i></a><a href="#" class="cancel-edit-btn text-muted" data-id="' + id + '" title="Cancel"><i class="ti ti-x"></i></a>');
});
...
<div class="row">
    {{ Form::open(['route' => ['branchpurchase.convert_to_invoice.store', $branchPurchase->id], 'method' => 'POST', 'class' => 'w-100 branchpurchase-convert-form', 'id' => 'branchpurchase-convert-form', 'novalidate' => true]) }}
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-light" onclick="closeActiveBootstrapModal();">{{ __('Cancel') }}</button>
