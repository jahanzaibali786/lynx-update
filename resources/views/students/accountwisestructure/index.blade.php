            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.status == 'success') {
                        $('#class_create').empty();
                        $('#class_create').append($('<option>', { value: '', text: 'Select Class' }));
                        for (var j = 0; j < result.class.length; j++) {
                            var cls = result.class[j];
                            $('#class_create').append($('<option>', { value: cls.id, text: cls.name }));
                        }
                    }
                }
            });
                    ...
                    <div class="row d-flex justify-content-end">
                        ...
                        <div class="col-auto float-end ms-2 mt-4">
                            ...
                            <a class="btn mx-1 btn-sm btn-outline-danger" onclick="detachAllChecked()">
                                ✕ Detach Checked <span id="checkedCount" class="badge bg-white text-danger ms-1" style="display:none;">0</span>
                            </a>
                            <a id="submitChecked" class="btn mx-1 btn-sm btn-outline-success" onclick="submitChanges()" style="display:none;">
                                Save Changes <span id="dirtyCount" class="badge bg-danger ms-1" style="display:none;">0</span>
                            </a>
                        </div>
                    </div>
                    {{ Form::close() }}
...
<style>
    ...
    .apply-to-all-link:hover { color: #0056b3; }

    /* Highlight dirty (modified) rows */
    tr.row-dirty {
        background-color: #fff8e1 !important;
    }
    tr.row-dirty td { border-left: 3px solid #ffc107; }
    ...
    .btn-detach:hover { background: #dc3545; color: #fff; }
</style>
            ...
            @foreach ($class_wise_fee as $fee)
                ...
                @foreach ($student as $student_fee)
                    ...
                    <tr id="row_{{ $rowCounter }}" data-original-amount="{{ @$fee->amount }}" data-original-discount="{{ @$student_fee->discount }}">
                        ...
                        <td>
                            <input type="text" id="netAmount_{{ $rowCounter }}" value="{{ @$discountAmount }}" readonly>
                        </td>
                        <td>
                            <input class="form-check-input active_checkbox" type="checkbox"
                                name="checked[]"
                                id="activeCheck_{{ $rowCounter }}"
                                onchange="markDirty({{ $rowCounter }})"
                                {{ @$student_fee->checked_status ? 'checked' : '' }}>

                            {{-- Hidden fields --}}
                            <input type="hidden" class="branch_id" id="brnch_{{ $rowCounter }}" value="{{ @$student_fee->student->enrollment->owned_by }}">
                            <input type="hidden" class="class_id" id="clsId_{{ $rowCounter }}" value="{{ @$fee->class_id }}">
                            <input type="hidden" class="student_id" id="stdid_{{ $rowCounter }}" value="{{ @$student_fee->student_id }}">
                            <input type="hidden" class="fee_val" value="{{ $rowCounter }}">
                            <input type="hidden" class="reg_student_id" id="stdregid_{{ $rowCounter }}" value="{{ @$student_fee->reg_id }}">
                            <input type="hidden" class="head_id" id="headid_{{ $rowCounter }}" value="{{ @$fee->head_id }}">
                        </td>
                        <td>
...
<script>
    ...
    function calculateNetAmount(id) {
        ...
        if (discount > 100) { discount = 100; discountField.value = 100; }
        if (discount < 0)   { discount = 0;   discountField.value = 0; }
    ...
    function applyDiscountToAll() {
        ...
        if (!firstDiscount) { alert('First discount field not found!'); return; }

        const val = parseFloat(firstDiscount.value) || 0;
        if (val > 100 || val < 0) { alert('Discount must be between 0 and 100'); return; }
    ...
    function detachAllChecked() {
        ...
        if (!confirm('Detach fee head from ' + checkedBoxes.length + ' checked student(s)?\n\nThis will permanently remove the fee head from all selected students. This cannot be undone.')) return;
            ...
            if (data.status === 'success') {
                ...
                payload.forEach(function(item) {
                    ...
                    if (row) {
                        ...
                        row.style.opacity    = '0';
                        setTimeout(() => { row.remove(); clearDirty(item.rowId); }, 350);
                    }
                });
                setTimeout(() => {
                    updateCheckedCount();
