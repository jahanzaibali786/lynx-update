...
<table style="margin-top: -80px !important;">
    <thead class="table_heads">
        <tr>
            ...
            <th style="font-size: 12px !important; font-family: Arial, Helvetica, sans-serif !important;">{{ __('Age') }}</th>
        </tr>
    </thead>
    <tbody>
            ...
            @foreach ($classesByBranch as $className => $enrollments)
                @foreach ($enrollments as $enroll)
                    <tr>
                        ...
                        <td style="font-size: 12px!important; font-family: Arial, Helvetica, sans-serif !important; width: 60px;">
                           @if($enroll->dob && $enroll->dob != '0000-00-00')
                                 {{ \Carbon\Carbon::parse($enroll->dob)->diff(now())->format('%y years %m months') }}
                            @else
                                -
                            @endif
                        </td>
