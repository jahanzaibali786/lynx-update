...
<head>
    ...
    <title>{{ \Auth::user()->stockTransferOrderNumberFormat($StockTransferOrder->branch_purchase_no) }}</title>
        <style>
    ...
    white-space: nowrap;
        ...
        @media print {
            ...
            .page { position: relative; left: auto; top: auto; width: 100% !important; min-height: 280mm; height: auto !important; overflow: visible !important; margin: 0; padding-bottom: 0; border: none !important; background: #ffffff !important; }
    ...
    <div class="page">  
        ...
        <div class="top-row">
            <div class="brand-block">
                ...
                <div class="note-line">STOCK TRANSFER REQUISITION</div>
            </div>
            <div class="right-head">
                <div class="invoice-title">STR</div>
                <table class="meta-box">
                    ...
                    <tr>
                        <td>{{ \App\Models\Utility::dateFormat($settings, $StockTransferOrder->purchase_date) }}</td>
                        <td>{{ \Auth::user()->stockTransferOrderNumberFormat($StockTransferOrder->branch_purchase_no) }}</td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="section-row">
            <div class="box">
                ...
                <div class="content">
                    <div>{{ $StockTransferOrder->branchUser->name ?? '-' }}</div>
                    <div class="small-hand">{{ $StockTransferOrder->branchUser->email ?? '-' }}</div>
                    <div class="small-hand">{{ $StockTransferOrder->branchUser->phone ?? '' }}</div>
                </div>
            </div>
            <div class="box">
                <div class="label">Details</div>
                <div class="content">
                    <div>{{ optional($StockTransferOrder->academicSession)->year ?? '-' }}</div>
                    <div>{{ $StockTransferOrder->category->name ?? '-' }}</div>
                    <div class="small-hand">{{ \App\Models\Utility::dateFormat($settings, $StockTransferOrder->purchase_date) }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="mid-table-wrap">
            <table class="mid-table">
                ...
                <tr>
                    <td class="topcell">STR #</td>
                    <td class="topcell">Session</td>
                    <td class="topcell">Purchase Date</td>
                    <td class="topcell">Branch</td>
                    <td class="topcell">Category</td>
                </tr>
                <tr>
                    <td class="center">{{ \Auth::user()->stockTransferOrderNumberFormat($StockTransferOrder->branch_purchase_no) }}</td>
                    <td class="center">{{ $session ?? '-' }}</td>
                    <td class="center">{{ \App\Models\Utility::dateFormat($settings, $StockTransferOrder->purchase_date) }}</td>
                    <td class="center">{{ $StockTransferOrder->branchUser->name ?? '-' }}</td>
                    <td class="center">{{ $StockTransferOrder->category->name ?? '-' }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="item-table-wrap">
            <table class="item-table">
                ...
                <tbody>
                    @php
                        $groupedPrintItems = $StockTransferOrder->items->groupBy(function ($item) {
                    ...
                    <tr>
                        <td colspan="5" style="text-align:left;border-left:1px solid #8b7768;font-weight:700;">
                            Total item {{ $StockTransferOrder->items->count() }} / Total quantity {{ number_format($StockTransferOrder->items->sum('quantity'), 0) }}</td>
                        <td style="text-align:center;font-weight:700;border-left:1px solid #8b7768;">Total</td>
                        <td style="text-align:right;font-weight:400;">{{ number_format($StockTransferOrder->getTotal(), 2) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="footer-line">
            ALL STOCK HAS BEEN REQUESTED AND VERIFIED BY THE BRANCH AS PER THE REQUISITION.
        </div>
        <div class="signature-wrap">
            <div class="sig"><b>Prepared By : </b> <span style="border-bottom:2px solid black;">{{ $branch->name ?? '_________________________' }}</span></div>
            <div class="sig right"><b>Approved By : </b> <span style="border-bottom:2px solid black;"> {{ \Auth::user()->name ?? '__________________________' }} </span></div>
            <div class="watermark"><div>{{ strtoupper(\App\Models\StockTransferOrder::$statues[$StockTransferOrder->status] ?? '') }}</div><div style="font-size: 20px;text-align: center;">{{ \App\Models\Utility::dateFormat($settings, $StockTransferOrder->issued_at ?? $StockTransferOrder->approve_date ?? $StockTransferOrder->rejected_at ?? $StockTransferOrder->forwarded_at ?? $StockTransferOrder->issue_date) }}</div></div>
        </div>
    </div>
</body>
