...
@section('content')
    ...
    <script>
        ...
        var existingItems = {!! json_encode($StockTransferOrder->items) !!};
        ...
        function appendInlineRow() {
            ...
            var row = '<tr data-row-id="' + rid + '" class="inline-edit-row">' +
                ...
                '<input type="number" class="form-control form-control-sm row-discount" data-rid="' + rid +
                '" placeholder="0.00" min="0" step="0.01" value="0" readonly>' +
                '</td>' +
        ...
        function calculateRowAmount(rid) {
            ...
            $('#amount-' + rid).text(((quantity * price) - discount).toFixed(2));
        }
        ...
        $(document).on('click', '.edit-entry-btn', function(e) {
            e.preventDefault();
            var id = parseInt($(this).data('id'));
            var entry = purchaseItems.find(function(item) {
                return item.id === id;
            });
            if (!entry) return;
        ...
        $(document).on('click', '.save-edit-btn', function(e) {
            ...
            entry.amount = (quantity * entry.price) - discount;
            tr.replaceWith(buildLockedRow(entry));
            renderHiddenInputs();
            updateTotals();
        });
        ...
        $(document).on('submit', '#stock-transfer-order-form', function(e) {
        ...
        $(document).ready(function() {
            if (typeof ajaxModalForm !== 'undefined') {
                ajaxModalForm({
                    formSelector: '.stock-transfer-order-ajax-form',
                    ...
                    onSuccess: function(r) {
                        $.ajax({
                            ...
                            success: function(html) {
                                ...
                                if (el) {
                                    document.getElementById('content-area').innerHTML = el
                                        .innerHTML;
                                    try {
                                        common_bind();
                                        commonLoader();
    ...
    <div class="row">
        {{ Form::model($StockTransferOrder, ['route' => ['stock-transfer-order.update', $StockTransferOrder->id], 'method' => 'PUT', 'class' => 'w-100 stock-transfer-order-ajax-form', 'id' => 'stock-transfer-order-form', 'novalidate' => true]) }}
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group" id="branch-box">
                                ...
                                {{ Form::select('branch_id', $branches, null, ['class' => 'form-control select', 'id' => 'branch_id', 'data-url' => route('stock-transfer-order.vender'), 'required' => 'required']) }}
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        {{ Form::label('purchase_number', __('Stock Transfer Order Number'), ['class' => 'form-label']) }}
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        {{ Form::label('purchase_date', __('Stock Transfer Order Date'), ['class' => 'form-label']) }}
            ...
            <div class="card" style="padding-bottom:100px!important;">
                ...
                <div id="items-table-wrap" class="card-body table-border-style pt-0">
                    <table class="table table-sm mb-0" id="items-table">
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="4" class="text-end"><strong>{{ __('Sub Total') }}
                                        ({{ \Auth::user()->currencySymbol() }})</strong></td>
                                <td class="text-end subTotal fw-bold">0.00</td>
                                <td></td>
                            </tr>
                            <tr>
                                <td colspan="4" class="text-end"><strong>{{ __('Discount') }}
                                        ({{ \Auth::user()->currencySymbol() }})</strong></td>
                                <td class="text-end totalDiscount">0.00</td>
                                <td></td>
                            </tr>
                            <tr>
                                <td colspan="4" class="text-end"><strong>{{ __('Total Amount') }}
                                        ({{ \Auth::user()->currencySymbol() }})</strong></td>
        ...
        <div class="modal-footer">
            <input type="button" value="{{ __('Cancel') }}"
                onclick="location.href = '{{ route('stock-transfer-order.index') }}';" class="btn btn-outline-light">
