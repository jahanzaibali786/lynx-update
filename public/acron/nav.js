...
class Nav {
    ...
    if (menuPlacement === 'vertical' && this.selectedMenuBehaviour === 'unpinned') {
      if (this.verticalMobile > windowWidth || this.verticalUnpinned <= windowWidth) {
        ...
        if (this.behaviourStatus !== 2) {
          if (this.verticalMobile > windowWidth) { // Check if it's not mobile
            this.html.setAttribute('data-behaviour', 'unpinned');
            this._disablePinButton(); // This line adds 'disabled' class to the pin button.
            this.behaviourStatus = 2;
            this._hideShowMenu();
          } else {
            this.html.setAttribute('data-behaviour', 'pinned'); // Adjust the behavior for mobile
            this._enablePinButton();
            this.behaviourStatus = 1;
            this._hideShowMenu();
          }
  ...
  _unCollapseMenu() {
    ...
    $(".dash-item.dash-hasmenu span.dash-mtext").show();
  }
  ...
  _hideDropdowns() {
    ...
    const dashMaintext = document.querySelector(".dash-item.dash-hasmenu span.dash-mtext")
    ...
    $(".dash-item.dash-hasmenu span.dash-mtext").hide();
  ...
  _onMouseEnter(event) {
    if (
      ...
      // First, make sure all dash-mtext elements are visible
      document.querySelectorAll(".dash-item.dash-hasmenu span.dash-mtext").forEach(el => {
        el.style.display = 'inline-block';
      });
      
      // Then, restore any previously opened menus
      document.querySelectorAll('#menu>li>a[data-clicked="true"]').forEach((element) => {
        if (element.nextElementSibling && element.nextElementSibling.tagName === 'UL') {
          // Instead of using Bootstrap's collapse, directly manipulate the DOM
  ...
  _onMouseLeave(event) {
    if (
      ...
      // Hide all submenus directly
      document.querySelectorAll('#menu>li>a').forEach((element) => {
        if (element.nextElementSibling && element.nextElementSibling.tagName === 'UL') {
          // Keep track of which ones were open
          const wasOpen = element.nextElementSibling.classList.contains('show');
          if (wasOpen) {
            // We don't remove data-clicked here so we remember it was open
